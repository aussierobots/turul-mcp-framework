...
pub fn derive_mcp_tool_impl(input: DeriveInput) -> Result<TokenStream> {
    ...
    let expanded = quote! {
        ...
        impl #name {
            ...
            fn generate_heuristic_schema(field_name: &str, struct_name: &str) -> mcp_protocol::tools::ToolSchema {
                use std::collections::HashMap;
                ...
                if struct_name.contains("Struct") || struct_name.contains("Complex") || struct_name.contains("Result") {
                    ...
                    let mut schema = mcp_protocol::tools::ToolSchema::object();
            ...
            fn update_schema_from_value(field_name: &str, value: &serde_json::Value) -> mcp_protocol::tools::ToolSchema {
                use std::collections::HashMap;
                use mcp_protocol::schema::JsonSchema;
            ...
            async fn call(&self, args: serde_json::Value, session: Option<mcp_server::SessionContext>) -> mcp_server::McpResult<mcp_protocol::tools::CallToolResult> {
                ...
                match instance.execute(session).await {
                    Ok(result) => {
                        ...
                        mcp_protocol::tools::CallToolResult::from_result_auto(&wrapped_result, self.output_schema())
                    }
                    Err(e) => Err(e)
                }
            }
