...
use turul_mcp_server::{McpResult, SessionContext, McpTool as McpToolTrait};
use turul_mcp_protocol::tools::{HasOutputSchema, HasBaseMetadata};
use serde_json::json;
...
async fn test_number_tool_execution_and_schema() {
    ...
    if let Some(structured) = response.structured_content {
        ...
        assert!(obj.contains_key("output"), "Expected 'output' field for primitive type, got keys: {:?}", obj.keys().collect::<Vec<_>>());
    ...
    assert_eq!(schema.schema_type, "object", "MCP requires all output schemas to be objects");
}

#[tokio::test]
async fn test_string_tool_execution_and_schema() {
    let tool = StringTool { text: "test".to_string() };
...
async fn test_object_tool_execution_and_schema() {
    ...
    // Test execution
    let result = tool.call(json!({"message": "test", "count": 42}), None).await;
    assert!(result.is_ok());
...
async fn test_all_tools_have_schemas_after_execution() {
    ...
    let string_tool = StringTool { text: "test".to_string() };
    let boolean_tool = BooleanTool { number: 1 };
    let array_tool = ArrayTool { size: 1 };
    let object_tool = ObjectTool { message: "test".to_string(), count: 1 };
    ...
    let _ = object_tool.call(json!({"message": "test", "count": 1}), None).await;
    
    // All should have output schemas
    assert!(number_tool.output_schema().is_some(), "NumberTool should have output schema");
    assert!(string_tool.output_schema().is_some(), "StringTool should have output schema");  
    assert!(boolean_tool.output_schema().is_some(), "BooleanTool should have output schema");
    assert!(array_tool.output_schema().is_some(), "ArrayTool should have output schema");
    assert!(object_tool.output_schema().is_some(), "ObjectTool should have output schema");
    
    // Verify all schema types are MCP-compliant objects
    assert_eq!(number_tool.output_schema().unwrap().schema_type, "object", "All MCP schemas must be objects");
    assert_eq!(string_tool.output_schema().unwrap().schema_type, "object", "All MCP schemas must be objects");
    assert_eq!(boolean_tool.output_schema().unwrap().schema_type, "object", "All MCP schemas must be objects"); 
    assert_eq!(array_tool.output_schema().unwrap().schema_type, "object", "All MCP schemas must be objects");
    assert_eq!(object_tool.output_schema().unwrap().schema_type, "object", "All MCP schemas must be objects");
}
...
async fn test_struct_output_uses_struct_name_as_field() {
    ...
    if let Some(structured) = response.structured_content {
        ...
        assert!(obj.contains_key("customResult"), "Expected 'customResult' field for struct output, got keys: {:?}", obj.keys().collect::<Vec<_>>());
        assert!(!obj.contains_key("result"), "Should not have 'result' field for struct output");
...
fn test_tools_have_output_schemas_before_execution() {
    ...
    let string_tool = StringTool { text: String::new() };
    let boolean_tool = BooleanTool { number: 0 };
    let array_tool = ArrayTool { size: 0 };
    let object_tool = ObjectTool { message: String::new(), count: 0 };
    
    // All tools should have output schemas for tools/list even before execution
    assert!(number_tool.output_schema().is_some(), "NumberTool should have output schema before execution");
    assert!(string_tool.output_schema().is_some(), "StringTool should have output schema before execution");  
    assert!(boolean_tool.output_schema().is_some(), "BooleanTool should have output schema before execution");
    assert!(array_tool.output_schema().is_some(), "ArrayTool should have output schema before execution");
    assert!(object_tool.output_schema().is_some(), "ObjectTool should have output schema before execution");
...
async fn test_output_schema_upgrades_after_execution() {
    ...
    if let Some(required_after) = &schema_after.required {
        assert!(!required_after.is_empty(), "Schema after execution should have required fields");
