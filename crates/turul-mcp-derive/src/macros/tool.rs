...
use quote::quote;
use syn::{parse::Parse, parse::ParseStream, Result, Token, Expr, Ident, Type, LitStr, LitBool, LitFloat, LitInt};
...
pub fn tool_declarative_impl(input: TokenStream) -> Result<TokenStream> {
    ...
    for param in &input.params {
        ...
        let base_schema = match type_str.as_str() {
            ...
            "bool" => quote! { turul_mcp_protocol::schema::JsonSchema::boolean().with_description(#param_desc) },
            "String" => quote! { turul_mcp_protocol::schema::JsonSchema::string().with_description(#param_desc) },
            s if s.starts_with("Option<") => {
                ...
                match inner_type_str {
                    "f64" | "f32" => quote! { turul_mcp_protocol::schema::JsonSchema::number().with_description(#param_desc) },
                    "i64" | "i32" | "i16" | "i8" | "u64" | "u32" | "u16" | "u8" | "isize" | "usize" => quote! { turul_mcp_protocol::schema::JsonSchema::integer().with_description(#param_desc) },
                    "bool" => quote! { turul_mcp_protocol::schema::JsonSchema::boolean().with_description(#param_desc) },
                    "String" => quote! { turul_mcp_protocol::schema::JsonSchema::string().with_description(#param_desc) },
                    _ => quote! { turul_mcp_protocol::schema::JsonSchema::string().with_description(#param_desc) },
                }
            },
            _ => quote! { turul_mcp_protocol::schema::JsonSchema::string().with_description(#param_desc) },
        };
