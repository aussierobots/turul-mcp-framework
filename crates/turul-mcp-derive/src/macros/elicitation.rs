//! Elicitation Declarative Macro Implementation
//!
//! Implements the `elicitation!{}` declarative macro for creating MCP elicitation handlers
...
/// Implementation function for the elicitation!{} declarative macro
pub fn elicitation_declarative_impl(input: TokenStream) -> Result<TokenStream> {
    let input = syn::parse::<ElicitationMacroInput>(input)?;
    let output = elicitation_declarative_impl_inner(input);
    Ok(output.into())
}
...
impl Parse for ElicitationMacroInput {
...
/// Individual field in elicitation macro
...
impl Parse for ElicitationField {
    fn parse(input: ParseStream) -> Result<Self> {
        let name_ident: Ident = input.parse()?;
        let name = name_ident.to_string();
...
fn generate_struct_fields(fields: &[ElicitationField]) -> proc_macro2::TokenStream {
    if fields.is_empty() {
        return quote! {};
    }
...
mod tests {
    ...
    fn test_elicitation_macro_parse() {
        ...
        assert!(code.contains("UserDetailsElicitation"));
        assert!(code.contains("McpElicitation"));
        assert!(code.contains("Please provide your information"));
    }
    ...
    fn test_elicitation_macro_unit_struct() {
        ...
        let result = elicitation_declarative_impl_inner(input);
        let code = result.to_string();
        assert!(code.contains("SimpleElicitation"));
        assert!(code.contains("Simple elicitation"));
    }
}
