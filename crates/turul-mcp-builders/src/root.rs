...
mod tests {
    ...
    fn test_root_builder_filtering() {
        ...
        assert_eq!(
            root.allowed_extensions(),
            Some(&["rs".to_string(), "toml".to_string()][..])
        );
        assert_eq!(
            root.excluded_patterns(),
            Some(&["target".to_string(), ".git".to_string()][..])
        );
        assert_eq!(
            root.tags(),
            Some(&["rust".to_string(), "source".to_string()][..])
        );
    }
    ...
    fn test_root_builder_convenience_extensions() {
        ...
        assert_eq!(
            root.tags(),
            Some(&["rust".to_string(), "project".to_string()][..])
        );
    }
