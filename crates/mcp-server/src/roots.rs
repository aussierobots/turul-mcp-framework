...
use mcp_protocol::{McpResult, roots::{ListRootsRequest, ListRootsResult, RootsListChangedNotification}};
...
pub trait McpRoot: RootDefinition + Send + Sync {
    ...
    async fn validate_path(&self, path: &str) -> McpResult<()> {
        ...
        if !canonical_path.starts_with(&canonical_root) {
            return Err(mcp_protocol::McpError::validation("Path is outside root directory"));
        }
