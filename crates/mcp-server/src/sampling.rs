...
use mcp_protocol::{McpResult, sampling::{CreateMessageRequest, CreateMessageResult}};
use mcp_protocol::sampling::SamplingDefinition;
...
pub trait McpSampling: SamplingDefinition + Send + Sync {
    ...
    async fn validate_request(&self, request: &CreateMessageRequest) -> McpResult<()> {
        ...
        if request.params.max_tokens == 0 {
            return Err(mcp_protocol::McpError::validation("max_tokens must be greater than 0"));
        }
        if request.params.max_tokens > 1000000 {
            return Err(mcp_protocol::McpError::validation("max_tokens exceeds maximum allowed value"));
        }
        Ok(())
    }
}
...
pub fn sampling_to_params(sampling: &dyn McpSampling) -> mcp_protocol::sampling::CreateMessageParams {
