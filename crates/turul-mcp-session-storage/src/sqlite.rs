...
impl SqliteSessionStorage {
    ...
    pub async fn with_config(config: SqliteConfig) -> Result<Self, SqliteError> {
        info!(
            "Initializing SQLite session storage at {:?}",
        ...
        if let Some(parent) = config.database_path.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                crate::sqlite::SqliteError::Connection(format!(
...
impl SessionStorage for SqliteSessionStorage {
    ...
    async fn update_session(&self, session_info: SessionInfo) -> Result<(), Self::Error> {
        ...
        if rows_affected == 0 {
            return Err(
                crate::sqlite::SqliteError::SessionNotFound(session_info.session_id).into(),
            );
        }
    ...
    async fn set_session_state(
        ...
        let current_state_json =
            sqlx::query_scalar::<_, String>("SELECT state FROM sessions WHERE session_id = ?")
                .bind(session_id)
                .fetch_optional(&self.pool)
                .await?
                .ok_or_else(|| {
                    crate::sqlite::SqliteError::SessionNotFound(session_id.to_string())
                })?;
    ...
    async fn expire_sessions(&self, older_than: SystemTime) -> Result<Vec<String>, Self::Error> {
        ...
        let expired_sessions: Vec<String> =
            sqlx::query_scalar("SELECT session_id FROM sessions WHERE last_activity < ?")
...
mod tests {
    ...
    async fn test_session_state_management() {
        ...
        // Set state
        storage
            .set_session_state(&session_id, "key1", json!("value1"))
            .await
            .unwrap();
        storage
            .set_session_state(&session_id, "key2", json!(42))
            .await
            .unwrap();

        // Get state
        let value1 = storage
            .get_session_state(&session_id, "key1")
            .await
            .unwrap();
        assert_eq!(value1, Some(json!("value1")));

        let value2 = storage
            .get_session_state(&session_id, "key2")
            .await
            .unwrap();
        ...
        // Remove state
        let removed = storage
            .remove_session_state(&session_id, "key1")
            .await
            .unwrap();
    ...
    async fn test_event_storage() {
        ...
        let events = storage
            .get_events_after(&session_id, stored_event1.id)
            .await
            .unwrap();
