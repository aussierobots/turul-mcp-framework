...
impl PostgresSessionStorage {
    ...
    pub async fn with_config(config: PostgresConfig) -> Result<Self, PostgresError> {
        info!(
            "Initializing PostgreSQL session storage at {}",
        ...
        let pool = sqlx::postgres::PgPoolOptions::new()
            ...
            .await
            .map_err(|e| {
                PostgresError::Connection(format!("Failed to connect to PostgreSQL: {}", e))
            })?;
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if config.enable_pooling_optimizations
                    && let Err(e) =
                        sqlx::query("REFRESH MATERIALIZED VIEW CONCURRENTLY session_stats")
                            .execute(&pool)
                            .await
                {
    ...
    async fn get_session_with_version(
        &self,
        session_id: &str,
    ) -> Result<Option<(SessionInfo, i32)>, PostgresError> {
        let row = sqlx::query(
            r#"
        ...
        match row {
            Some(row) => {
                let client_capabilities: Option<ClientCapabilities> = row
                    .get::<Option<Value>, _>("client_capabilities")
...
impl SessionStorage for PostgresSessionStorage {
    ...
    async fn update_session(&self, session_info: SessionInfo) -> Result<(), Self::Error> {
        let client_capabilities_json = session_info
            .client_capabilities
            .as_ref()
            .map(serde_json::to_value)
            .transpose()?;

        let server_capabilities_json = session_info
            .server_capabilities
            .as_ref()
            .map(serde_json::to_value)
            .transpose()?
    ...
    async fn expire_sessions(&self, older_than: SystemTime) -> Result<Vec<String>, Self::Error> {
        ...
        let expired_sessions: Vec<String> =
            sqlx::query_scalar("SELECT session_id FROM sessions WHERE last_activity < $1")
    ...
    async fn maintenance(&self) -> Result<(), Self::Error> {
        ...
        if deleted_sessions > 0 {
            info!(
                "Maintenance: Cleaned up {} expired sessions",
                deleted_sessions
            );
        }
...
mod tests {
    ...
    async fn test_jsonb_state_operations() {
        ...
        storage
            .set_session_state(&session_id, "user", json!({"name": "Alice", "age": 30}))
            .await
            .unwrap();
        storage
            .set_session_state(
        ...
        // Get state
        let user = storage
            .get_session_state(&session_id, "user")
            .await
            .unwrap();
        ...
        // Remove state
        let removed = storage
            .remove_session_state(&session_id, "user")
            .await
            .unwrap();
        ...
        let prefs = storage
            .get_session_state(&session_id, "preferences")
            .await
            .unwrap();
    ...
    async fn test_concurrent_operations() {
        ...
        let handles: Vec<_> = (0..10)
            .map(|i| {
                ...
                tokio::spawn(async move {
                    storage
                        .set_session_state(&session_id, &format!("key_{}", i), json!(i))
                        .await
                })
            })
            .collect();
        ...
        for i in 0..10 {
            let value = storage_clone
                .get_session_state(&session_id, &format!("key_{}", i))
                .await
                .unwrap();
