...
impl LoggingBuilder {
    ...
    pub fn structured(level: LoggingLevel, fields: HashMap<String, Value>) -> Self {
        Self::new(level, serde_json::to_value(fields).unwrap_or(Value::Object(serde_json::Map::new())))
    }
...
impl LoggingBuilder {
    ...
    pub fn exception(
        ...
        if let Some(stack) = stack_trace {
            data.insert("stack_trace".to_string(), Value::String(stack));
        }
        if let Some(ctx) = context {
            data.extend(ctx);
        }
        Self::structured(LoggingLevel::Error, data)
    }
    ...
    pub fn performance(
        ...
        duration_ms: f64,
        additional_metrics: Option<HashMap<String, Value>>
    ) -> Self {
        let mut data = HashMap::new();
        data.insert("operation".to_string(), Value::String(operation.into()));
        data.insert("duration_ms".to_string(), Value::from(duration_ms));
        if let Some(metrics) = additional_metrics {
            data.extend(metrics);
        }
        Self::structured(LoggingLevel::Info, data)
    }
    ...
    pub fn security(
        ...
        data.insert("event_type".to_string(), Value::String(event_type.into()));
        data.insert("category".to_string(), Value::String("security".to_string()));
        Self::structured(severity, data)
    }
    ...
    pub fn audit(
        ...
        let mut data = HashMap::new();
        data.insert("user".to_string(), Value::String(user.into()));
        data.insert("action".to_string(), Value::String(action.into()));
        if let Some(res) = resource {
            data.insert("resource".to_string(), Value::String(res));
        }
        data.insert("outcome".to_string(), Value::String(outcome.into()));
        data.insert("category".to_string(), Value::String("audit".to_string()));
        Self::structured(LoggingLevel::Notice, data)
    }

    /// Log a trace event for debugging
    pub fn trace(
        ...
        let mut data = HashMap::new();
        data.insert("function".to_string(), Value::String(function.into()));
        if let Some(arguments) = args {
            data.insert("args".to_string(), serde_json::to_value(arguments).unwrap_or(Value::Null));
        }
        if let Some(res) = result {
            data.insert("result".to_string(), res);
        }
        data.insert("category".to_string(), Value::String("trace".to_string()));
        Self::structured(LoggingLevel::Debug, data)
    }
}
...
mod tests {
    use super::*;
    use serde_json::json;
    use mcp_protocol::logging::LoggerDefinition;
    ...
    fn test_with_context_logging() {
        ...
        if let Value::Object(data_obj) = &notification.params.data {
            assert_eq!(data_obj.get("message"), Some(&json!("User logged in successfully")));
    ...
    fn test_exception_logging() {
        let mut context = HashMap::new();
        context.insert("function".to_string(), json!("process_request"));
        context.insert("user_id".to_string(), json!("user-456"));

        let notification = LoggingBuilder::exception(
            "Database connection failed",
            Some("at line 123 in db.rs".to_string()),
            Some(context)
        ).build();

        assert_eq!(notification.params.level, LoggingLevel::Error);
        
        if let Value::Object(data_obj) = &notification.params.data {
            assert_eq!(data_obj.get("error"), Some(&json!("Database connection failed")));
            assert_eq!(data_obj.get("stack_trace"), Some(&json!("at line 123 in db.rs")));
            assert_eq!(data_obj.get("function"), Some(&json!("process_request")));
            assert_eq!(data_obj.get("user_id"), Some(&json!("user-456")));
        } else {
            panic!("Expected object data");
        }
    }
    ...
    fn test_performance_logging() {
        ...
        metrics.insert("memory_used".to_string(), json!(256.5));
        metrics.insert("cpu_percent".to_string(), json!(12.3));

        let notification = LoggingBuilder::performance(
            "database_query",
            45.2,
            Some(metrics)
        ).build();
        ...
        if let Value::Object(data_obj) = &notification.params.data {
            assert_eq!(data_obj.get("operation"), Some(&json!("database_query")));
            assert_eq!(data_obj.get("duration_ms"), Some(&json!(45.2)));
            assert_eq!(data_obj.get("memory_used"), Some(&json!(256.5)));
            assert_eq!(data_obj.get("cpu_percent"), Some(&json!(12.3)));
        } else {
            panic!("Expected object data");
        }
    }
    ...
    fn test_audit_logging() {
        let notification = LoggingBuilder::audit(
            "admin_user",
            "delete_resource",
            Some("file://sensitive.txt".to_string()),
            "success"
        ).build();
        ...
        if let Value::Object(data_obj) = &notification.params.data {
            assert_eq!(data_obj.get("user"), Some(&json!("admin_user")));
            assert_eq!(data_obj.get("action"), Some(&json!("delete_resource")));
            assert_eq!(data_obj.get("resource"), Some(&json!("file://sensitive.txt")));
            assert_eq!(data_obj.get("outcome"), Some(&json!("success")));
            assert_eq!(data_obj.get("category"), Some(&json!("audit")));
        } else {
            panic!("Expected object data");
        }
    }
    ...
    fn test_log_format_with_string_data() {
        let logger = LoggingBuilder::text(LoggingLevel::Info, "Simple string message")
            .build_dynamic();
    ...
    fn test_log_format_with_object_data() {
        ...
        let logger = LoggingBuilder::new(LoggingLevel::Info, data)
            .build_dynamic();
