...
use mcp_protocol::sampling::{
    SamplingMessage, Role, ModelPreferences, ModelHint, CreateMessageParams, CreateMessageRequest
};
use mcp_protocol::prompts::ContentBlock;
...
impl MessageBuilder {
    ...
    pub fn system(mut self, content: impl Into<String>) -> Self {
        self.messages.push(SamplingMessage {
            role: Role::System,
            content: ContentBlock::Text { text: content.into() },
        });
        self
    }
...
impl ModelPreferencesBuilder {
    ...
    pub fn build(self) -> ModelPreferences {
        ModelPreferences {
            hints: if self.hints.is_empty() { None } else { Some(self.hints) },
...
mod tests {
    ...
    fn test_message_builder_model_preferences() {
        ...
        let preferences = params.model_preferences.expect("Expected model preferences");
        assert_eq!(preferences.hints.as_ref().unwrap().len(), 1);
        assert_eq!(preferences.hints.as_ref().unwrap()[0], ModelHint::Claude35Sonnet20241022);
    ...
    fn test_message_builder_complete_request() {
        ...
        assert_eq!(request.params.system_prompt, Some("You are a coding assistant".to_string()));
