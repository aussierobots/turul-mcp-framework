...
impl McpHandler for PromptsHandler {
    async fn handle(&self, params: Option<Value>) -> McpResult<Value> {
        ...
        use turul_mcp_protocol::meta::{PaginatedResponse, Cursor};
...
pub trait McpPrompt: Send + Sync {
    ...
    async fn generate(&self, args: HashMap<String, Value>) -> McpResult<Vec<turul_mcp_protocol::prompts::PromptMessage>>;
}
...
impl McpHandler for LoggingHandler {
    async fn handle(&self, params: Option<Value>) -> McpResult<Value> {
        use turul_mcp_protocol::logging::SetLevelRequest;
...
impl McpHandler for TemplatesHandler {
    async fn handle(&self, _params: Option<Value>) -> McpResult<Value> {
        ...
        debug!("Templates handler called - returning empty list as templates are now integrated into resources");
        ...
        let base_response = turul_mcp_protocol::resources::ListResourceTemplatesResult::new(templates);
...
pub trait ElicitationProvider: Send + Sync {
    ...
    async fn elicit(&self, request: &turul_mcp_protocol_2025_06_18::elicitation::ElicitCreateRequest) -> McpResult<turul_mcp_protocol_2025_06_18::elicitation::ElicitResult>;
    
    /// Check if this provider can handle a specific elicitation schema
    fn can_handle(&self, _request: &turul_mcp_protocol_2025_06_18::elicitation::ElicitCreateRequest) -> bool {
    ...
    async fn handle(&self, params: Option<Value>) -> McpResult<Value> {
        ...
        if let Some(params) = params {
            ...
            if !self.provider.can_handle(&create_request) {
                let error_response = turul_mcp_protocol_2025_06_18::elicitation::ElicitResult::cancel();
    ...
    async fn handle_with_session(&self, params: Option<Value>, session: Option<SessionContext>) -> McpResult<Value> {
        ...
        if let Some(session_ctx) = session {
            ...
            if let (Some(client_info_value), Some(capabilities_value), Some(negotiated_version_value)) = 
                ...
                use turul_mcp_protocol::{Implementation, ClientCapabilities, McpVersion};
                ...
                if let (Ok(client_info), Ok(client_capabilities), Ok(negotiated_version)) = (
                    ...
                    if let Err(e) = self.session_manager.initialize_session_with_version(
                        ...
                        return Err(turul_mcp_protocol::McpError::configuration(&format!("Failed to initialize session: {}", e)));
                    }
                    ...
                    return Err(turul_mcp_protocol::McpError::configuration("Failed to deserialize stored client info"));
                }
            } else {
                tracing::error!("❌ Missing stored client info/capabilities/version for session {}", session_ctx.session_id);
                return Err(turul_mcp_protocol::McpError::configuration("Missing stored client info - session must call initialize first"));
            }
        } else {
