...
mod tests {
    ...
    use crate::security::{SecurityMiddleware, RateLimitConfig, ResourceAccessControl, AccessLevel, InputValidator};
    ...
    async fn test_security_with_valid_uri_template() {
        ...
        let mut access_control = ResourceAccessControl::default();
        access_control.allowed_patterns = vec![
            Regex::new(r"^file:///secure/[a-zA-Z0-9_-]+/data\.json$").unwrap(),
        ];
        access_control.access_level = AccessLevel::SessionRequired;
        ...
        let result = read_handler.handle_with_session(Some(params), Some(session)).await;
    ...
    async fn test_rate_limiting_integration() {
        ...
        let result1 = read_handler.handle_with_session(Some(params.clone()), Some(session.clone())).await;
        if result1.is_err() {
            println!("First request failed: {:?}", result1);
        }
        assert!(result1.is_ok());
        
        let result2 = read_handler.handle_with_session(Some(params.clone()), Some(session.clone())).await;
        if result2.is_err() {
        ...
        let result3 = read_handler.handle_with_session(Some(params), Some(session)).await;
        if result3.is_ok() {
    ...
    async fn test_input_validation_integration() {
        ...
        let result = read_handler.handle_with_session(Some(malicious_params), Some(session)).await;
        assert!(result.is_err());
    }
    ...
    async fn test_mime_type_and_size_validation() {
        ...
        impl HasResourceMetadata for LargeContentResource {
            fn name(&self) -> &str { "large_content" }
        }
        impl HasResourceUri for LargeContentResource {
            fn uri(&self) -> &str { &self.base_pattern }
        }
        impl HasResourceDescription for LargeContentResource {
            fn description(&self) -> Option<&str> { Some("Large content resource") }
        }
        impl HasResourceMimeType for LargeContentResource {
            fn mime_type(&self) -> Option<&str> { Some("application/octet-stream") }
        }
        ...
        impl HasResourceAnnotations for LargeContentResource {
            fn annotations(&self) -> Option<&meta::Annotations> { None }
        }
        ...
        let mut access_control = ResourceAccessControl::default();
        access_control.allowed_patterns = vec![Regex::new(r"^file:///large\.bin$").unwrap()];
        access_control.max_size = Some(500); // Smaller than the 1000 character response
        ...
        let read_handler = ResourcesReadHandler::new()
            .with_security(Arc::new(security_middleware))
            .add_template_resource(template, LargeContentResource {
