...
mod state_management_tests {
    ...
    async fn test_basic_state_operations() {
        ...
        // Set state
        manager.set_session_state(&session_id, "test_key", json!("test_value")).await;
        ...
        // Update state
        manager.set_session_state(&session_id, "test_key", json!("updated_value")).await;
        let value = manager.get_session_state(&session_id, "test_key").await;
        ...
        let removed = manager.remove_session_state(&session_id, "non_existent").await;
    ...
    async fn test_complex_state_types() {
        ...
        let test_cases = vec![
            ...
            ("object", json!({"nested": {"value": 123}, "array": [1, 2, 3]})),
            ("null", json!(null)),
        ];
        
        // Set all values
        for (key, value) in &test_cases {
            manager.set_session_state(&session_id, key, value.clone()).await;
        }
    ...
    async fn test_multiple_session_state_isolation() {
        ...
        manager.set_session_state(&session1, "key", json!("session1_value")).await;
        manager.set_session_state(&session2, "key", json!("session2_value")).await;
    ...
    async fn test_state_operations_on_nonexistent_session() {
        ...
        manager.set_session_state(fake_session_id, "key", json!("value")).await;
...
mod initialization_tests {
    ...
    async fn test_session_initialization() {
        ...
        let result = manager.initialize_session(&session_id, client_info.clone(), client_capabilities.clone()).await;
        assert!(result.is_ok());
    ...
    async fn test_initialization_of_nonexistent_session() {
        ...
        let result = manager.initialize_session("non-existent", client_info, client_capabilities).await;
        assert!(matches!(result, Err(SessionError::NotFound(_))));
    }
...
mod event_broadcasting_tests {
    ...
    async fn test_send_event_to_specific_session() {
        ...
        let result = manager.send_event_to_session("non-existent", SessionEvent::KeepAlive).await;
        assert!(matches!(result, Err(SessionError::NotFound(_))));
    }
...
mod concurrency_tests {
    ...
    async fn test_concurrent_session_creation() {
        ...
        for id in &session_ids {
            assert!(unique_ids.insert(id.clone()), "Duplicate session ID: {}", id);
        }
    ...
    async fn test_concurrent_state_operations() {
        ...
        for i in 0..num_operations {
            ...
            let handle = tokio::spawn(async move {
                ...
                // Set state
                manager_clone.set_session_state(&session_id_clone, &key, value.clone()).await;
                
                // Get state
                let retrieved = manager_clone.get_session_state(&session_id_clone, &key).await;
                assert_eq!(retrieved, Some(value));
                
                // Remove state
                let removed = manager_clone.remove_session_state(&session_id_clone, &key).await;
...
mod error_handling_tests {
    ...
    async fn test_invalid_session_operations() {
        ...
        let removed_value = manager.remove_session_state(invalid_session_id, "key").await;
        assert_eq!(removed_value, None);
        
        let event_result = manager.send_event_to_session(invalid_session_id, SessionEvent::KeepAlive).await;
...
mod performance_tests {
    ...
    async fn test_large_session_count() {
        ...
        for session_id in &session_ids {
            assert!(manager.session_exists(session_id).await);
            manager.set_session_state(session_id, "test", json!("value")).await;
        }
