...
use turul_mcp_protocol::{
    ServerCapabilities,
    logging::{HasLogFormat, HasLogLevel, HasLoggingMetadata, LoggingLevel},
};
...
mod session_aware_logger_tests {
    ...
    async fn test_all_logging_levels_with_session_aware_logger() {
        ...
        let test_cases = vec![
            ...
            (
                LoggingBuilder::critical(json!("Critical message")),
                "critical",
            ),
            (LoggingBuilder::alert(json!("Alert message")), "alert"),
            (
                LoggingBuilder::emergency(json!("Emergency message")),
                "emergency",
            ),
        ];
    ...
    async fn test_complex_message_formatting() {
        ...
        let test_cases = vec![
            ...
            (
                json!({"error": "Database connection failed", "code": 500}),
                "{\"code\":500,\"error\":\"Database connection failed\"}",
            ),
