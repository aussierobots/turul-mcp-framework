...
/// Method: "notifications/tasks/status" (per MCP 2025-11-25)
///
/// Sent by the server when a task's status changes.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct TaskStatusNotification {
    /// Method name (always "notifications/tasks/status")
    pub method: String,
    /// Task status parameters (Task fields flattened)
...
pub struct TaskStatusNotificationParams {
    /// The full task object with current status
    #[serde(flatten)]
...
impl TaskStatusNotification {
    pub fn new(task: crate::tasks::Task) -> Self {
        Self {
...
mod tests {
    ...
    fn test_task_status_notification() {
        ...
        assert_eq!(json["params"]["taskId"], "task-abc");
        assert_eq!(json["params"]["status"], "working");
        assert_eq!(json["params"]["createdAt"], "2025-01-01T00:00:00Z");
    }
