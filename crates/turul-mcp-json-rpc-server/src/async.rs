...
pub trait JsonRpcHandler: Send + Sync {
    ...
    async fn handle(&self, method: &str, params: Option<RequestParams>, session_context: Option<SessionContext>) -> Result<Value, Self::Error>;

    /// Handle a JSON-RPC notification with optional session context (optional - default does nothing)
    async fn handle_notification(&self, method: &str, params: Option<RequestParams>, session_context: Option<SessionContext>) -> Result<(), Self::Error> {
...
where
    ...
    F: Fn(&str, Option<RequestParams>, Option<SessionContext>) -> futures::future::BoxFuture<'static, Result<Value, E>> + Send + Sync,
    N: Fn(&str, Option<RequestParams>, Option<SessionContext>) -> futures::future::BoxFuture<'static, Result<(), E>> + Send + Sync,
{
    handler_fn: F,
...
mod tests {
    ...
    impl JsonRpcHandler for TestHandler {
        ...
        async fn handle(&self, method: &str, _params: Option<RequestParams>, _session_context: Option<SessionContext>) -> Result<Value, Self::Error> {
            match method {
