...
- [ ] **DynamoDB session storage uses snake_case attribute names** — `turul-mcp-session-storage` hardcodes snake_case DynamoDB attribute names (`session_id`, `created_at`, `last_activity`, `is_initialized`, `event_id`) in `dynamodb.rs`. DynamoDB convention is camelCase (`sessionId`, `createdAt`, `lastActivity`, `isInitialized`, `eventId`). The framework also auto-creates tables with snake_case keys when `create_tables_if_missing: true`. Fix: use serde `#[serde(rename_all = "camelCase")]` to map Rust snake_case fields to DynamoDB camelCase attributes, like the rest of the GPS Trust project does. **Migration plan:** Add serde `#[serde(alias = "snake_case_name")]` on each field so reads accept both old snake_case and new camelCase from DynamoDB. New writes go out as camelCase. Existing tables (`gps-trust-mcp-sessions`, `sv-track-mcp-sessions`) continue working without data migration — old snake_case rows are read via alias, new rows are written as camelCase. Key schema (`session_id` → `sessionId`, `event_id` → `eventId`) requires table recreation since DynamoDB keys can't be renamed. Sequence: (1) add `rename_all = "camelCase"` + aliases on non-key attributes, (2) create new tables with camelCase keys, (3) update `create_tables_if_missing` to use camelCase keys, (4) update downstream SAM templates (`sv-track`, `gps-trust-mcp`). (`crates/turul-mcp-session-storage/src/dynamodb.rs`, lines 740-798 `session_to_dynamodb_item`, lines 626-650 `ensure_events_table_exists`)
