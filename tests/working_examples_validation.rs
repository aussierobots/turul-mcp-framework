...
async fn test_working_examples_compilation() {
    ...
    println!(
        "Testing compilation of {} examples in parallel...",
        example_count
    );
    ...
    let compile_futures = working_examples.into_iter().map(|example| {
        async move {
            ...
            match result {
                ...
                Ok(Err(e)) => Err(format!(
                    "Failed to execute cargo check for {}: {}",
                    example, e
                )),
                Err(_) => Err(format!(
                    "Compilation check for {} timed out after 60 seconds",
                    example
                )),
            }
        }
    ...
    match test_result {
        Ok(output) => {
            ...
            if stdout.contains("🎆 FULLY MCP COMPLIANT") {
                ...
                panic!(
                    "MCP compliance test did not show success message:\n{}",
                    stdout
                );
            }
        }
