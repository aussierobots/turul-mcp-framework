...
#[tool(
    name = "test_derive_with_session",
    description = "Test derive macro with SessionContext"
)]
...
impl TestDeriveWithSession {
    async fn execute(&self, session: Option<SessionContext>) -> McpResult<Value> {
        let session = session
            .ok_or_else(|| McpError::SessionError("Session required for testing".to_string()))?;

        // Test state management
        let count: i32 = session.get_typed_state("call_count").await.unwrap_or(0);
        let new_count = count + 1;
        session
            .set_typed_state("call_count", &new_count)
            .await
            .unwrap();
...
#[derive(McpTool, Default)]
#[tool(
    name = "test_derive_no_session",
    description = "Test derive macro without SessionContext"
)]
...
#[mcp_tool(
    name = "test_function_with_session",
    description = "Test function macro with SessionContext"
)]
async fn test_function_with_session(
    ...
    let session = session
        .ok_or_else(|| McpError::SessionError("Session required for function test".to_string()))?;

    // Test state management in function macro
    let prefix: String = session
        .get_typed_state("prefix")
        .await
...
#[mcp_tool(
    name = "test_function_no_session",
    description = "Test function macro without SessionContext"
)]
...
mod tests {
    ...
    async fn test_derive_macro_with_session_context() {
        ...
        let result = tool
            .call(args.clone(), Some(session.clone()))
            .await
            .unwrap();
    ...
    async fn test_session_state_persistence_across_tools() {
        ...
        let result1 = derive_tool
            .call(args1, Some(session.clone()))
            .await
            .unwrap();
        ...
        let result2 = function_tool
            .call(args2, Some(session.clone()))
            .await
            .unwrap();
