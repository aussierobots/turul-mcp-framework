...
async fn test_tool_with_output_schema_must_have_structured_content() {
    ...
    let result = tool.call(json!({"text": "hello world"}), None).await.unwrap();
...
async fn test_tool_without_output_schema_doesnt_need_structured_content() {
    ...
    impl turul_mcp_protocol::tools::HasBaseMetadata for SimpleTextTool {
        fn name(&self) -> &str { "simple_text" }
        fn title(&self) -> Option<&str> { None }
    }

    impl turul_mcp_protocol::tools::HasDescription for SimpleTextTool {
        fn description(&self) -> Option<&str> { Some("Simple text tool") }
    }
    ...
    impl turul_mcp_protocol::tools::HasToolMeta for SimpleTextTool {
        fn tool_meta(&self) -> Option<&std::collections::HashMap<String, Value>> { None }
    }
    ...
    assert!(result.content.len() > 0, "Tool must provide content");
...
async fn test_mcp_tool_macro_compliance() {
    ...
    assert!(result.content.len() > 0, "Macro-generated tool must provide content");
...
async fn test_mcp_tool_custom_output_field_compliance() {
    ...
    let result = tool.call(json!({"text": "hello world test"}), None).await.unwrap();
...
async fn test_derive_macro_compliance() {
    ...
    // Check runtime output uses custom field name
    let result = tool.call(json!({"text": "hello world test derive"}), None).await.unwrap();
    assert!(result.structured_content.is_some());
    if let Some(structured) = &result.structured_content {
        assert!(
        ...
        assert!(count_result["message"].as_str().unwrap().contains("4 words"));
    }
}
