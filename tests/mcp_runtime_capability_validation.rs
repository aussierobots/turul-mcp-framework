...
use turul_mcp_derive::{mcp_tool, McpPrompt};
use turul_mcp_server::prelude::*;

/// Simple test tool for capability validation
#[mcp_tool(name = "test_tool", description = "Test tool for capability validation")]
...
#[derive(McpPrompt)]
#[prompt(name = "test_prompt", description = "Test prompt for capability validation")]
struct TestPrompt {
...
async fn test_prompts_capability_truthfulness() {
    ...
    let server_handle = tokio::spawn(async move {
        let server = McpServer::builder()
            ...
            .version("1.0.0")
            .prompt(TestPrompt { input: "test".to_string() })
            .bind_address(format!("127.0.0.1:{}", port).parse().unwrap())
            .build()
            .expect("Failed to build server");
...
async fn test_json_rpc_protocol_compliance() {
    ...
    assert!(body["error"].is_null() || body.get("error").is_none(), "Should not have error on success");
    
    // Validate MCP protocol version
    assert_eq!(body["result"]["protocolVersion"], "2025-06-18", "Must support MCP 2025-06-18");
    
    // Validate server info structure
    let server_info = &body["result"]["serverInfo"];
    assert!(server_info["name"].is_string(), "Server name must be string");
    assert!(server_info["version"].is_string(), "Server version must be string");
...
mod integration {
    ...
    async fn test_full_mcp_compliance_integration() {
        ...
        if let Some(tools) = capabilities.get("tools") {
            assert_eq!(tools["listChanged"], false, "Static framework: tools.listChanged must be false");
        }
        
        if let Some(prompts) = capabilities.get("prompts") {
            assert_eq!(prompts["listChanged"], false, "Static framework: prompts.listChanged must be false");
        }
        
        if let Some(resources) = capabilities.get("resources") {
            assert_eq!(resources["listChanged"], false, "Static framework: resources.listChanged must be false");
            // Resources subscribe should be false until implemented
            assert_eq!(resources["subscribe"], false, "resources.subscribe must be false until implemented");
        }
