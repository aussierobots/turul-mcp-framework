...
async fn test_tools_server_startup_and_discovery() {
    ...
    assert_eq!(server_info.get("name").unwrap().as_str().unwrap(), "tools-test-server");
...
async fn test_tools_list_endpoint() {
    ...
    let tool_names: Vec<&str> = tools.iter()
        .map(|t| t.as_object().unwrap().get("name").unwrap().as_str().unwrap())
        .collect();
    ...
    for expected_tool in expected_tools {
        assert!(tool_names.contains(&expected_tool), "Missing tool: {}", expected_tool);
    }
...
async fn test_calculator_tool_execution() {
    ...
    // Test addition
    let add_result = client.call_tool("calculator", json!({
        "operation": "add",
        "a": 5.0,
        "b": 3.0
    })).await.expect("Failed to call calculator tool");
    ...
    if let Some(calc_result_val) = TestFixtures::extract_tool_result_object(&add_result) {
        ...
        assert_eq!(calc_result.get("operation").unwrap().as_str().unwrap(), "add");
    } else {
        panic!("No tool result found");
    }

    // Test division by zero error
    let div_zero_result = client.call_tool("calculator", json!({
        "operation": "divide",
        "a": 10.0,
        "b": 0.0
    })).await;
...
async fn test_string_processor_tool() {
    ...
    let result = client.call_tool("string_processor", json!({
        "text": "hello world",
        "operation": "uppercase"
    })).await.expect("Failed to call string processor");

    let parsed_result = TestFixtures::extract_tool_result_object(&result).expect("No tool result found");
    
    assert_eq!(parsed_result.get("result").unwrap().as_str().unwrap(), "HELLO WORLD");
    assert_eq!(parsed_result.get("operation").unwrap().as_str().unwrap(), "uppercase");
    assert_eq!(parsed_result.get("original").unwrap().as_str().unwrap(), "hello world");
    ...
    assert!(parsed_result.get("result").unwrap().as_str().unwrap().contains("4 characters"));
...
async fn test_data_transformer_tool() {
    ...
    let result = client.call_tool("data_transformer", json!({
        "data": test_data,
        "operation": "extract_keys"
    })).await.expect("Failed to call data transformer");
...
async fn test_session_counter_tool_state_management() {
    ...
    // Test increment
    let result = client.call_tool("session_counter", json!({
        "operation": "increment",
        "amount": 5
    })).await.expect("Failed to call session counter");

    let parsed_result = TestFixtures::extract_tool_result_object(&result).expect("No tool result found");
    
    assert_eq!(parsed_result.get("current_value").unwrap().as_i64().unwrap(), 5);
    assert_eq!(parsed_result.get("session_id").unwrap().as_str().unwrap(), session_id);
    ...
    assert_eq!(parsed_result.get("current_value").unwrap().as_i64().unwrap(), 3);
...
async fn test_progress_tracker_with_notifications() {
    ...
    let result = client.call_tool("progress_tracker", json!({
    ...
    assert_eq!(parsed_result.get("operation").unwrap().as_str().unwrap(), "progress_tracker");
    assert_eq!(parsed_result.get("duration").unwrap().as_f64().unwrap(), 2.0);
    assert_eq!(parsed_result.get("steps").unwrap().as_i64().unwrap(), 4);
    assert_eq!(parsed_result.get("status").unwrap().as_str().unwrap(), "completed");
                ...
                if let Ok(text) = String::from_utf8(chunk.to_vec()) {
                    if !text.trim().is_empty() {
                        ...
                        if text.contains("\"method\":\"notifications/progress\"") {
                            ...
                            info!("✅ Progress notification received via SSE: {}", progress_events.len());
                        }
                    }
                }
            }
    ...
    info!("✅ SSE client-side verification complete: {} progress notifications received", progress_events.len());
    
    // Validate structure of all received progress notifications
    for (i, event) in progress_events.iter().enumerate() {
        assert!(event.contains("\"method\":\"notifications/progress\""), 
               "Progress notification {} has invalid method format: {}", i, event);
        assert!(event.contains("\"progress\""), 
               "Progress notification {} missing progress field: {}", i, event);
        assert!(event.contains("\"progressToken\""), 
               "Progress notification {} missing progressToken field: {}", i, event);
    }
...
async fn test_error_generator_tool() {
    ...
    let result = client.call_tool("error_generator", json!({
    ...
    match result {
        Ok(result_val) => {
            if result_val.contains_key("error") {
                ...
                info!("Got success response for validation error: {:#?}", result_val);
            }
        },
        Err(e) => {
            info!("✅ Error generator validation correctly returned HTTP error: {:?}", e);
        }
    }
...
async fn test_notifications_initialized_lifecycle() {
    ...
    let response = client.send_notification(initialized_notification)
        .await
    ...
    let calc_result = client.call_tool("calculator", json!({
        "operation": "add",
        "a": 5.0,
        "b": 3.0
    })).await.expect("Failed to call calculator after initialized notification");
...
async fn test_notifications_tools_list_changed_compliance() {
    ...
    debug!("Initialize result for tools listChanged test: {:?}", init_result);
    ...
    for tool in &tools {
        let tool_obj = tool.as_object().unwrap();
        assert!(tool_obj.contains_key("name"), "Tool missing required 'name' field");
        assert!(tool_obj.contains_key("description"), "Tool missing required 'description' field");
        assert!(tool_obj.contains_key("inputSchema"), "Tool missing required 'inputSchema' field");
    }
    ...
    match response {
        Ok(result) => debug!("✅ Server handled tools/listChanged notification: {:?}", result),
    ...
    if list_changed_events.is_empty() {
        ...
        info!("ℹ️  Received {} tools/listChanged events (testing infrastructure)", list_changed_events.len());
        
        for (i, event) in list_changed_events.iter().enumerate() {
            assert!(event.contains("\"method\":\"notifications/tools/listChanged\""), 
                   "Event {} has incorrect method format: {}", i, event);
            assert!(event.contains("\"jsonrpc\":\"2.0\""), 
                   "Event {} missing JSON-RPC version: {}", i, event);
        }
    }

    // Step 7: Verify tools list consistency after notification handling
    let tools_result_after = client.list_tools().await.expect("Failed to list tools after notification");
    let tools_after = TestFixtures::extract_tools_list(&tools_result_after).expect("Failed to extract tools after notification");
    
    // Tool list should be identical (static framework)
    assert_eq!(tools.len(), tools_after.len(), 
...
async fn test_parameter_validator_tool() {
    ...
    let result = client.call_tool("parameter_validator", json!({
        "email": "test@example.com",
        "age": 25,
        "config": {"setting": "value"},
        "tags": ["tag1", "tag2"]
    })).await.expect("Failed to call parameter validator");

    let parsed_result = TestFixtures::extract_tool_result_object(&result).expect("No tool result found");
    
    assert_eq!(parsed_result.get("validation_result").unwrap().as_str().unwrap(), "passed");
    assert_eq!(parsed_result.get("email").unwrap().as_str().unwrap(), "test@example.com");
...
async fn test_tools_protocol_compliance() {
    ...
    let init_result = client.initialize_with_capabilities(TestFixtures::tools_capabilities()).await.unwrap();
    ...
    assert_eq!(tools_result.get("jsonrpc").unwrap().as_str().unwrap(), "2.0");

    // Test tools/call compliance  
    let call_result = client.call_tool("calculator", json!({
        "operation": "add",
        "a": 1.0,
        "b": 2.0
    })).await.unwrap();
...
async fn test_concurrent_tool_execution() {
    ...
    let tasks = vec![
        tokio::spawn({
            ...
            async move {
                client.call_tool("calculator", json!({"operation": "add", "a": 1.0, "b": 2.0})).await
            }
        }),
        tokio::spawn({
            let client = client.clone();
            async move {
                client.call_tool("string_processor", json!({"text": "test", "operation": "uppercase"})).await
            }
        }),
        tokio::spawn({
            let client = client.clone();
            async move {
                client.call_tool("session_counter", json!({"operation": "increment", "amount": 1})).await
            }
        }),
    ];
    ...
    for result in results {
        assert!(result.is_ok(), "Concurrent tool execution failed: {:?}", result);
    }
...
async fn test_session_storage_integration() {
    ...
    client1.initialize_with_capabilities(TestFixtures::tools_capabilities()).await.expect("Failed to initialize client1");
    client2.initialize_with_capabilities(TestFixtures::tools_capabilities()).await.expect("Failed to initialize client2");
    ...
    info!("✅ Session isolation verified: {} vs {}", session1_id, session2_id);
    ...
    let result = client1.call_tool("session_counter", json!({
        "operation": "increment",
        "amount": 5
    ...
    let parsed_result = TestFixtures::extract_tool_result_object(&result).expect("No tool result found");
    assert_eq!(parsed_result.get("current_value").unwrap().as_i64().unwrap(), 5);
    assert_eq!(parsed_result.get("session_id").unwrap().as_str().unwrap(), session1_id);
    ...
    let parsed_result = TestFixtures::extract_tool_result_object(&result).expect("No tool result found");
    assert_eq!(parsed_result.get("current_value").unwrap().as_i64().unwrap(), 8);
    
    // Test session state persistence - Client 2 (should start at 0)
    info!("🔍 Testing session isolation for client 2...");
    
    let result = client2.call_tool("session_counter", json!({
        "operation": "get"
    })).await.expect("Failed to get counter");
    
    let parsed_result = TestFixtures::extract_tool_result_object(&result).expect("No tool result found");
    assert_eq!(parsed_result.get("current_value").unwrap().as_i64().unwrap(), 0, "Client 2 should start with 0");
    assert_eq!(parsed_result.get("session_id").unwrap().as_str().unwrap(), session2_id);
    ...
    let parsed_result = TestFixtures::extract_tool_result_object(&result).expect("No tool result found");
    assert_eq!(parsed_result.get("current_value").unwrap().as_i64().unwrap(), 10);
    ...
    let parsed_result = TestFixtures::extract_tool_result_object(&result).expect("No tool result found");
    assert_eq!(parsed_result.get("current_value").unwrap().as_i64().unwrap(), 8, "Client 1 state should be unchanged");
    ...
    let parsed_result = TestFixtures::extract_tool_result_object(&result).expect("No tool result found");
    assert_eq!(parsed_result.get("current_value").unwrap().as_i64().unwrap(), 10, "Client 2 state should be maintained");
    ...
    let result = client1.call_tool("session_counter", json!({
        "operation": "decrement",
        "amount": 3
    })).await.expect("Failed to decrement counter");
    ...
    let result = client1.call_tool("session_counter", json!({
        "operation": "reset"
    })).await.expect("Failed to reset counter");
    
    let parsed_result = TestFixtures::extract_tool_result_object(&result).expect("No tool result found");
    assert_eq!(parsed_result.get("current_value").unwrap().as_i64().unwrap(), 0);
    ...
    let parsed_result = TestFixtures::extract_tool_result_object(&result).expect("No tool result found");
    assert_eq!(parsed_result.get("current_value").unwrap().as_i64().unwrap(), 10, "Client 2 should be unaffected by client 1 reset");
