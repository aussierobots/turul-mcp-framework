...
async fn test_large_json_parameter_handling() {
    ...
    for i in 0..1000 {
        ...
        section.insert("name".to_string(), json!(format!("Configuration section {}", i)));
    ...
    info!("🔍 Testing large payload: {} bytes ({:.1} MB)", payload_size, payload_size as f64 / 1024.0 / 1024.0);
    ...
    match result {
        Ok(response) => {
            if let Some(result_obj) = TestFixtures::extract_tool_result_object(&response) {
                ...
                info!("✅ Large payload processed successfully: {} keys found", key_count);
                assert!(key_count > 1000, "Should have processed many keys from large payload");
                
                // Performance check: should complete within reasonable time
                assert!(duration.as_secs() < 30, "Large payload processing took too long: {:?}", duration);
            } else {
...
async fn test_very_long_string_parameters() {
    ...
    info!("🔍 Testing very long string: {} characters", long_text.len());
    ...
    match result {
        Ok(response) => {
            if let Some(result_obj) = TestFixtures::extract_tool_result_object(&response) {
                ...
                info!("✅ Long string processed successfully: {} characters", text_length);
                assert_eq!(text_length, 1024 * 1024, "Length should match input string");
                
                // Performance check
                assert!(duration.as_secs() < 10, "Long string processing took too long: {:?}", duration);
            } else {
                warn!("⚠️  Tool handled long string but returned unexpected format");
            }
        }
        Err(e) => {
            info!("ℹ️  Long string rejected by server (acceptable for memory protection): {:?}", e);
        }
    }
}
    ...
    match result {
        Ok(response) => {
            if let Some(result_obj) = TestFixtures::extract_tool_result_object(&response) {
                ...
                info!("✅ Deep structure processed successfully: {} levels deep", max_depth);
                assert!(max_depth >= 50, "Should detect significant nesting depth");
                
                // Performance check
                assert!(duration.as_secs() < 5, "Deep structure processing took too long: {:?}", duration);
            } else {
                warn!("⚠️  Tool handled deep structure but returned unexpected format");
            }
        }
        Err(e) => {
            info!("ℹ️  Deep structure rejected (acceptable for stack protection): {:?}", e);
        }
    }
}
...
async fn test_large_array_parameters() {
    ...
    info!("🔍 Testing large array: {} items, {} bytes", large_array.len(), payload_size);
    ...
    match result {
        Ok(response) => {
            if let Some(result_obj) = TestFixtures::extract_tool_result_object(&response) {
                ...
                info!("✅ Large array processed successfully: {} items", item_count);
                assert_eq!(item_count, 10000, "Should count all array items");
                
                // Performance check
                assert!(duration.as_secs() < 15, "Large array processing took too long: {:?}", duration);
            } else {
                warn!("⚠️  Tool handled large array but returned unexpected format");
            }
        }
        Err(e) => {
            info!("ℹ️  Large array rejected (acceptable for memory protection): {:?}", e);
        }
    }
}
...
async fn test_unicode_and_special_characters() {
    ...
    info!("🔍 Testing Unicode text: {} characters", complex_text.chars().count());
        ...
        Ok(response) => {
            if let Some(result_obj) = TestFixtures::extract_tool_result_object(&response) {
                ...
                if let Some(processed_text) = result_obj.get("text") {
                    ...
                    assert!(processed_str.contains("Hello World"), "Basic ASCII should be preserved");
                    assert!(processed_str.contains("café"), "Accented characters should be preserved");
                    assert!(processed_str.contains("🚀"), "Emoji should be preserved");
                    assert!(processed_str.contains("你好"), "CJK characters should be preserved");
                }
            } else {
...
async fn test_message_size_limits_and_graceful_degradation() {
    ...
    for (size, label) in test_sizes {
        ...
        match result {
            ...
            Err(e) => {
                ...
                let recovery_test = client.call_tool("calculator", json!({
                    "operation": "add",
                    "a": 1.0,
                    "b": 1.0
                })).await;
                ...
                match recovery_test {
                    ...
                    Err(recovery_error) => {
                        warn!("⚠️  Server may have issues after size rejection: {:?}", recovery_error);
    ...
    info!("   Max successful size: {} bytes ({:.1} MB)", max_successful_size, max_successful_size as f64 / 1024.0 / 1024.0);
    info!("   Failures (size limits): {}", failures);
    
    // Should handle at least 100KB successfully
    assert!(max_successful_size >= 100 * 1024, 
           "Server should handle at least 100KB messages, max was: {}", max_successful_size);
}