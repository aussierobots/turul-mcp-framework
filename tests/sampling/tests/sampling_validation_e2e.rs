...
async fn test_sampling_large_message_handling() {
    ...
    let result = client
        .make_request("sampling/createMessage", large_request, 30)
        .await;
    ...
    match result {
        Ok(response) => {
            if response.contains_key("result") {
                let generated_text =
                    extract_sampling_message(&response).expect("Should extract message text");

                assert!(
                    !generated_text.is_empty(),
                    "Generated text should not be empty for large input"
                );
                info!(
                    "✅ Large message ({} chars) processed successfully",
                    large_content.len()
                );
                ...
                info!(
                    "ℹ️  Large message rejected (acceptable for memory protection): {}",
                    message
                );
            }
        }
        Err(e) => {
            info!(
                "ℹ️  Large message rejected at HTTP level (acceptable): {:?}",
                e
            );
        }
    }
...
async fn test_sampling_multiple_user_messages() {
    ...
    let result = client
        .make_request("sampling/createMessage", conversation_request, 31)
        .await
    ...
    assert!(
        !generated_text.is_empty(),
        "Should generate response to conversation"
    );
...
async fn test_sampling_edge_case_parameters() {
    ...
    for (case_name, request) in edge_cases {
        let result = client
            .make_request("sampling/createMessage", request, 33)
            .await;
        ...
        match result {
            ...
            Err(e) => {
                info!(
                    "ℹ️  Edge case '{}' rejected at HTTP level: {:?}",
                    case_name, e
                );
            }
        }
...
async fn test_sampling_malformed_messages() {
    ...
    for (case_name, request) in malformed_cases {
        let result = client
            .make_request("sampling/createMessage", request, 34)
            .await;
        ...
        match result {
            Ok(response) => {
                if response.contains_key("error") {
                    ...
                    let message = error.get("message").unwrap().as_str().unwrap();
                    info!(
                        "✅ Malformed case '{}' properly rejected: {}",
                        case_name, message
                    );
                } else {
...
async fn test_sampling_concurrent_requests() {
    ...
    for (i, client) in clients.into_iter().enumerate() {
        ...
        let handle = tokio::spawn(async move {
            client
                .make_request("sampling/createMessage", request, 35 + i as u64)
                .await
        });
...
async fn test_sampling_unicode_content() {
    ...
    let result = client
        .make_request("sampling/createMessage", unicode_request, 38)
        .await
    ...
    assert!(
        !generated_text.is_empty(),
        "Should generate response for Unicode content"
    );
    info!("✅ Unicode content processed successfully");
    debug!(
        "Unicode response preview: {}",
        &generated_text[..std::cmp::min(50, generated_text.len())]
    );
}
...
async fn test_sampling_session_isolation() {
    ...
    let result1 = client1
        .make_request("sampling/createMessage", request1, 39)
        .await
        .expect("Session 1 request should succeed");

    let result2 = client2
        .make_request("sampling/createMessage", request2, 40)
        .await
        .expect("Session 2 request should succeed");

    // Both sessions should work independently
    assert!(
        result1.contains_key("result"),
        "Session 1 should get result"
    );
    assert!(
        result2.contains_key("result"),
        "Session 2 should get result"
    );
    ...
    debug!(
        "Session 1 response: {}",
        &text1[..std::cmp::min(30, text1.len())]
    );
    debug!(
        "Session 2 response: {}",
        &text2[..std::cmp::min(30, text2.len())]
    );
}
