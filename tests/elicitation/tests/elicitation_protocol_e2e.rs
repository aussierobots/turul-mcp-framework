...
use mcp_elicitation_tests::{McpTestClient, TestServerManager, TestFixtures, json, debug, info};
use mcp_elicitation_tests::test_utils::{elicitation_capabilities, validate_workflow_structure};

#[tokio::test]
async fn test_elicitation_onboarding_workflow() {
    let _ = tracing_subscriber::fmt::try_init();

    let server = TestServerManager::start_elicitation_server().await.expect("Failed to start elicitation server");
    let mut client = McpTestClient::new(server.port());

    // Initialize with tool capabilities (elicitation uses tools)
    client.initialize_with_capabilities(elicitation_capabilities()).await.unwrap();

    // List available tools to find onboarding workflow tool
    let tools_result = client.make_request("tools/list", json!({}), 80).await
    ...
    let onboarding_tool = tools.iter()
        .find(|tool| {
            tool.as_object()
    ...
    let tool_name = onboarding_tool.as_object()
        .and_then(|obj| obj.get("name"))
        .and_then(|name| name.as_str())
        .unwrap();
    ...
    let workflow_result = client.call_tool(tool_name, json!({
        "workflow_type": "personal_account",
        "step_index": 0
    })).await.expect("Onboarding workflow should start");
    ...
    let business_workflow = client.call_tool(tool_name, json!({
...
async fn test_elicitation_compliance_forms() {
    ...
    let tools_result = client.make_request("tools/list", json!({}), 81).await
        .expect("Failed to list tools");
    ...
    let compliance_tool = tools.iter()
        .find(|tool| {
            tool.as_object()
                .and_then(|obj| obj.get("name"))
                .and_then(|name| name.as_str())
    ...
    if let Some(tool) = compliance_tool {
        ...
        let gdpr_result = client.call_tool(tool_name, json!({
        ...
        let ccpa_result = client.call_tool(tool_name, json!({
...
async fn test_elicitation_preference_collection() {
    ...
    let tools_result = client.make_request("tools/list", json!({}), 82).await
        .expect("Failed to list tools");
    ...
    let preference_tool = tools.iter()
        .find(|tool| {
            tool.as_object()
                .and_then(|obj| obj.get("name"))
                .and_then(|name| name.as_str())
    ...
    if let Some(tool) = preference_tool {
        ...
        let notification_result = client.call_tool(tool_name, json!({
        ...
        let accessibility_result = client.call_tool(tool_name, json!({
...
async fn test_elicitation_customer_surveys() {
    ...
    let tools_result = client.make_request("tools/list", json!({}), 83).await
        .expect("Failed to list tools");
    ...
    if let Some(tool) = survey_tool {
        ...
        let survey_result = client.call_tool(tool_name, json!({
        ...
        for segment in segments {
            let segment_result = client.call_tool(tool_name, json!({
                "survey_type": "customer_satisfaction",
...
async fn test_elicitation_data_validation() {
    ...
    let tools_result = client.make_request("tools/list", json!({}), 84).await
        .expect("Failed to list tools");
    ...
    let validation_tool = tools.iter()
        .find(|tool| {
            tool.as_object()
                .and_then(|obj| obj.get("name"))
                .and_then(|name| name.as_str())
    ...
    if let Some(tool) = validation_tool {
        ...
        for category in validation_categories {
            let validation_result = client.call_tool(tool_name, json!({
...
async fn test_elicitation_tool_schemas() {
    ...
    let tools_result = client.make_request("tools/list", json!({}), 85).await
        .expect("Failed to list tools");
    ...
    for (i, tool) in tools.iter().enumerate() {
        ...
        assert!(tool_obj.contains_key("description"), "Tool {} missing description", i);
        assert!(tool_obj.contains_key("inputSchema"), "Tool {} missing inputSchema", i);
        ...
        assert!(input_schema.contains_key("type"), "Tool {} schema missing type", name);
        assert_eq!(input_schema.get("type").unwrap().as_str().unwrap(), "object", 
                  "Tool {} schema should be object type", name);
        ...
        if input_schema.contains_key("properties") {
            ...
            assert!(!properties.is_empty(), "Tool {} should have properties if specified", name);
        }
    ...
    let elicitation_tool_names: Vec<&str> = tools.iter()
        .filter_map(|tool| {
            tool.as_object()
                .and_then(|obj| obj.get("name"))
                .and_then(|name| name.as_str())
    ...
    assert!(!elicitation_tool_names.is_empty(), 
           "Should have at least one elicitation-related tool");
    
    info!("✅ Found {} elicitation tools: {:?}", 
          elicitation_tool_names.len(), elicitation_tool_names);
}
...
async fn test_elicitation_server_capabilities() {
    ...
    let init_result = client.initialize_with_capabilities(elicitation_capabilities()).await.unwrap();
    ...
    if let Some(server_capabilities) = init_result.get("capabilities") {
        ...
        assert!(server_capabilities.get("tools").is_some(), 
        ...
        if let Some(elicitation_cap) = server_capabilities.get("elicitation") {
            info!("✅ Server advertises elicitation capabilities: {:?}", elicitation_cap);
        } else {
    ...
    let tools_result = client.make_request("tools/list", json!({}), 86).await
        .expect("Tools should be available");

    assert!(tools_result.contains_key("result"), "Should have tools result");
