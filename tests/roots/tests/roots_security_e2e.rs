...
async fn test_roots_access_control_boundaries() {
    ...
    let roots_result = client
        .make_request("roots/list", json!({}), 70)
        .await
        .expect("Failed to list roots");

    let roots = extract_roots_list(&roots_result).expect("Should extract roots list");

    assert!(
        !roots.is_empty(),
        "Should have at least one root for boundary testing"
    );

    // Extract allowed root URIs
    let allowed_roots: Vec<String> = roots
        .iter()
        .filter_map(|root| {
            root.as_object()
    ...
    let test_cases = vec![
        (
            ...
            format!(
                ...
                allowed_roots
                    .first()
                    .unwrap_or(&"file:///workspace".to_string())
            ),
            true,
        ),
        (
        ...
        (
            "traversal attack",
            "file:///workspace/../../../etc/passwd".to_string(),
            false,
        ),
    ];

    for (case_name, test_path, should_be_allowed) in test_cases {
        let is_allowed = is_uri_in_allowed_roots(
            &test_path,
            &allowed_roots.iter().map(|s| s.as_str()).collect::<Vec<_>>(),
        );
...
async fn test_roots_file_operation_security() {
    ...
    let tools_result = client
        .make_request("tools/list", json!({}), 71)
        .await
    ...
    let file_operation_tool = tools.iter().find(|tool| {
        tool.as_object()
        ...
        match allowed_operation {
            Ok(response) => {
                if let Some(content) = TestFixtures::extract_tool_result_object(&response) {
                    ...
                    assert!(
                        content_str.contains("✅") || content_str.contains("Success"),
            ...
            Err(e) => {
                info!(
                    "ℹ️  Allowed operation failed (may not be implemented): {:?}",
                    e
                );
            }
        }
...
async fn test_roots_path_traversal_protection() {
    ...
    // Get available tools
    let tools_result = client
        .make_request("tools/list", json!({}), 72)
        .await
        .expect("Failed to list tools");
    ...
    // Look for file operation tools
    let file_tool = tools.iter().find(|tool| {
        tool.as_object()
            .and_then(|obj| obj.get("name"))
            .and_then(|name| name.as_str())
            .map(|name| name.contains("file") || name.contains("simulate"))
    ...
    if let Some(tool) = file_tool {
        ...
        for attack_path in traversal_attacks {
            ...
            match result {
                    ...
                    if response.contains_key("error") {
                        ...
                        assert!(
                            message.to_lowercase().contains("access")
                                || message.to_lowercase().contains("denied")
                                || message.to_lowercase().contains("outside"),
                            "Should block traversal attack: {}",
                            attack_path
                        );
                        info!("✅ Path traversal attack blocked: {}", attack_path);
                    } else if let Some(content) =
                        TestFixtures::extract_tool_result_object(&response)
                    {
                        ...
                        {
                            info!(
                                "✅ Path traversal attack blocked in result: {}",
                                attack_path
                            );
                        } else {
                            // This should not happen - traversal should be blocked
                            warn!(
                                "⚠️  Path traversal attack may have succeeded: {}",
                                attack_path
                            );
                        }
                    }
                ...
                Err(_e) => {
                    info!(
                        "✅ Path traversal attack rejected at HTTP level: {}",
                        attack_path
                    );
                }
            }
...
async fn test_roots_permission_levels() {
    ...
    // Get available tools
    let tools_result = client
        .make_request("tools/list", json!({}), 73)
        .await
        .expect("Failed to list tools");
        ...
        for (test_name, path, operation, should_succeed) in permission_tests {
            ...
            match result {
                Ok(response) => {
                    ...
                    let success_in_result = if let Some(content) =
                        TestFixtures::extract_tool_result_object(&response)
                    {
                        let content_str = content.to_string();
                ...
                Err(e) => {
                    if should_succeed {
                        info!(
                            "ℹ️  {} failed at HTTP level (may not be implemented): {:?}",
                            test_name, e
                        );
                    } else {
...
async fn test_roots_uri_validation() {
    ...
    let roots_result = client
        .make_request("roots/list", json!({}), 74)
        .await
        .expect("Failed to list roots");
    ...
    for (i, root) in roots.iter().enumerate() {
        ...
        assert!(
            uri.contains("://"),
            "Root {} should have valid URI scheme: {}",
            i,
            uri
        );
        ...
        if uri.contains(" ") {
            assert!(
                uri.contains("%20"),
                "Root {} with spaces should be percent-encoded: {}",
                i,
                uri
            );
        }
        ...
        if let Some(path_part) = uri.strip_prefix("file://") {
            ...
            assert!(
                ...
                "Root {} file URI should have absolute path: {}",
                i,
                uri
            );
        }
...
async fn test_roots_security_headers() {
    ...
    let roots_result = client
        .make_request("roots/list", json!({}), 75)
        .await
        .expect("Failed to list roots");
    ...
    for pattern in sensitive_patterns {
        assert!(
            !response_str.to_lowercase().contains(pattern),
