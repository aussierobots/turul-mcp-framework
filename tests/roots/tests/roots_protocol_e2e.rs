...
async fn test_roots_list_endpoint() {
    ...
    let roots_result = client.make_request("roots/list", json!({}), 50).await
        .expect("Failed to list roots");

    debug!("Roots result: {:?}", roots_result);

    // Verify response structure
    assert!(roots_result.contains_key("result"), "Response should contain 'result'");
    let result = roots_result.get("result").unwrap().as_object().unwrap();

    // Check for roots array
    assert!(result.contains_key("roots"), "Result should contain 'roots'");
    ...
    for (i, root) in roots.iter().enumerate() {
        assert!(validate_root_object(root), "Root {} should have valid structure", i);
        
        let root_obj = root.as_object().unwrap();
        let uri = root_obj.get("uri").unwrap().as_str().unwrap();
        let name = root_obj.get("name").map(|n| n.as_str().unwrap()).unwrap_or("unnamed");
        
        // Verify URI has valid scheme
        assert!(uri.contains("://"), "Root {} URI should have valid scheme: {}", i, uri);
...
async fn test_roots_list_with_pagination() {
    ...
    let paginated_result = client.make_request("roots/list", json!({
        "cursor": "test_cursor"
    }), 51).await.expect("Failed to list roots with cursor");
...
async fn test_roots_structure_validation() {
    ...
    let roots_result = client.make_request("roots/list", json!({}), 52).await
        .expect("Failed to list roots");
    ...
    for root in roots {
        ...
        assert!(uri.starts_with("file://"), "URI should have file:// scheme: {}", uri);
        ...
        if let Some(name) = root_obj.get("name") {
            ...
            assert!(!name_str.is_empty(), "Root name should not be empty if present");
        }
    ...
    assert!(workspace_found || data_found || config_found, 
...
async fn test_roots_json_rpc_compliance() {
    ...
    let roots_result = client.make_request("roots/list", json!({}), 53).await
        .expect("Failed to list roots");

    // Verify JSON-RPC 2.0 compliance
    assert!(roots_result.contains_key("jsonrpc"), "Response should contain 'jsonrpc'");
    assert_eq!(roots_result.get("jsonrpc").unwrap().as_str().unwrap(), "2.0",
              "JSON-RPC version should be 2.0");

    assert!(roots_result.contains_key("id"), "Response should contain 'id'");
    assert_eq!(roots_result.get("id").unwrap().as_i64().unwrap(), 53,
              "Response ID should match request ID");

    assert!(roots_result.contains_key("result"), "Response should contain 'result'");
    assert!(!roots_result.contains_key("error"), "Successful response should not contain 'error'");
...
async fn test_roots_capability_advertising() {
    ...
    let init_result = client.initialize_with_capabilities(roots_capabilities()).await.unwrap();
    ...
    match test_result {
        ...
        Err(e) => {
            panic!("Roots endpoint should be available if server is running: {:?}", e);
        }
    }
}
...
async fn test_roots_error_handling() {
    ...
    let invalid_result = client.make_request("roots/list", json!({
    ...
    let recovery_test = client.make_request("roots/list", json!({}), 56).await
...
async fn test_roots_concurrent_access() {
        ...
        match handle.await {
            Ok(Ok(response)) => {
                if response.contains_key("result") {
                    ...
                    info!("✅ Concurrent request {} succeeded with {} roots", i + 1, roots.len());
                }
            }
            Ok(Err(e)) => {
    ...
    info!("📊 Concurrent roots access: {} successful requests", successes);
}