...
#[tool(name = "session_test", description = "Test SessionContext in derive macro")]
...
#[mcp_tool(name = "session_function_test", description = "Test SessionContext in function macro")]
async fn session_function_test(
    #[param(description = "Test input")] input: String,
    session: Option<SessionContext>
) -> McpResult<String> {
    Ok(format!("input: {}, has_session: {}", 
        input, 
        session.is_some()
    ))
}
...
mod tests {
    ...
    async fn test_function_macro_session_support() {
        ...
        assert!(response["result"].as_str().unwrap().contains("has_session: false"));
    }
}