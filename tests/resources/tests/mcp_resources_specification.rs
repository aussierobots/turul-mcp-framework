...
use turul_mcp_protocol::resources::{ResourceContent, ResourceTemplate, Resource};
// Removed unused imports - will add back when implementing pagination/progress features
use mcp_resources_tests::{UserProfileResource, AppConfigResource, LogFilesResource};
...
async fn test_resource_uri_rfc_6570_template_compliance() {
    ...
    for uri in &[user_resource.uri(), config_resource.uri(), log_resource.uri()] {
...
async fn test_resource_template_mcp_specification() {
    ...
    assert_eq!(user_template.mime_type.as_ref().unwrap(), "application/json");
...
async fn test_resource_polymorphism_mcp_compliance() {
    ...
    for resource in &resources {
        ...
        assert_eq!(resource_struct.description, resource.description().map(String::from));
    }
}
...
async fn test_resource_mcp_error_handling_compliance() {
    ...
    let json_rpc_error = resource_error.to_json_rpc_error();
