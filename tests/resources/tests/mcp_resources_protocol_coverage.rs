...
use turul_mcp_protocol::meta::{Cursor, Annotations};
...
async fn test_resource_template_rfc_6570_compliance() {
    ...
    let template = ResourceTemplate::new("multi_param", "file:///projects/{project_id}/files/{file_name}.{ext}")
        .with_description("Project files with multiple parameters")
        .with_annotations(Annotations::new().with_title("Project Files"));
    
    assert_eq!(template.uri_template, "file:///projects/{project_id}/files/{file_name}.{ext}");
...
async fn test_resource_struct_complete_fields() {
    ...
    assert_eq!(resource.title, Some("Complete Resource Example".to_string()));
    assert_eq!(resource.description, Some("A resource demonstrating all available fields".to_string()));
...
async fn test_list_resources_request_params() {
    ...
    assert_eq!(request.params.cursor.as_ref().unwrap().as_str(), "next_page_token_123");
}
...
async fn test_list_resources_result_pagination() {
    ...
    assert_eq!(result.next_cursor.as_ref().unwrap().as_str(), "page_2_token");
...
async fn test_read_resource_request_params() {
    ...
    let request = ReadResourceRequest::new("file:///user/456.json")
        .with_meta(meta);
...
async fn test_read_resource_result_multiple_contents() {
    ...
    let contents = vec![
        ...
        ResourceContent::blob("file:///user/789/avatar.jpg", "base64imagedata", "image/jpeg"),
    ];
...
async fn test_subscription_requests() {
    ...
    let subscribe_request = SubscribeRequest::new("file:///monitor/config.json")
        .with_meta(subscribe_meta);
    ...
    assert_eq!(unsubscribe_request.params.uri, "file:///monitor/config.json");
}
...
async fn test_resource_trait_implementations() {
    ...
    assert_eq!(resource.description(), Some("Test resource for trait validation"));
...
async fn test_serialization_round_trip_all_structs() {
    ...
    // ResourceTemplate
    let template = ResourceTemplate::new("test", "file:///test/{id}")
...
async fn test_edge_cases_and_robustness() {
    ...
    let minimal_blob = ResourceContent::blob("file:///minimal.bin", "AA==", "application/octet-stream");
    match minimal_blob {
