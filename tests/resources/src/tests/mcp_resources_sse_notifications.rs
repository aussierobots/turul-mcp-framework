...
async fn test_sse_event_type_mapping_correctness() {
    ...
    let test_cases = vec![
        (ResourceListChangedNotification::new().method, "notifications/resources/listChanged"),
        (ToolListChangedNotification::new().method, "notifications/tools/listChanged"),
        (PromptListChangedNotification::new().method, "notifications/prompts/listChanged"),
        (RootsListChangedNotification::new().method, "notifications/roots/listChanged"),
        (ProgressNotification::new("test".to_string(), 0).method, "notifications/progress"),
    ];
...
async fn test_notification_with_meta_structure() {
    ...
    assert_eq!(json_value["params"]["_meta"]["correlation_id"], "uuid-v7-test");
...
async fn test_notification_method_name_constants() {
    ...
    let expected_methods = vec![
        ("resources/listChanged", ResourceListChangedNotification::new().method),
        ("tools/listChanged", ToolListChangedNotification::new().method),
        ("prompts/listChanged", PromptListChangedNotification::new().method),
        ("roots/listChanged", RootsListChangedNotification::new().method),
        ("resources/updated", ResourceUpdatedNotification::new("test://uri".to_string()).method),
        ("progress", ProgressNotification::new("token".to_string(), 0).method),
    ];
