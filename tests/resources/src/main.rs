...
use mcp_resources_tests::{
    AppConfigResource, BinaryDataResource, FileUserResource, LogFilesResource, UserAvatarResource,
    UserProfileResource,
};
use tracing::info;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let profile_data = user_resource.fetch_profile_data("demo_id").await?;
    info!(
        "✅ User profile business logic: {} content items",
        profile_data.len()
    );

    let config_data = config_resource.fetch_config_data().await?;
    info!(
        "✅ Config business logic: {} content items",
        config_data.len()
    );
    ...
    info!(
        "✅ FileUserResource(123): {} TextResourceContent items",
        user_123_data.len()
    );
    info!(
        "✅ FileUserResource(456): {} TextResourceContent items",
        user_456_data.len()
    );
    ...
    info!(
        "✅ UserAvatarResource(123): {} BlobResourceContent items",
        avatar_123_data.len()
    );
    info!(
        "✅ UserAvatarResource(456): {} BlobResourceContent items",
        avatar_456_data.len()
    );
