...
use mcp_prompts_tests::{
    ReviewCodePrompt, GenerateDocsPrompt, AnalyzeErrorPrompt, PlanProjectPrompt,
    MultiContentPrompt, TemplateVarPrompt
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let review_prompt = ReviewCodePrompt::new("rust", "fn hello() { println!(\"Hello MCP!\"); }")
        .with_focus("security").with_target_level("senior");
    let docs_prompt = GenerateDocsPrompt::new("api", "pub fn add(a: i32, b: i32) -> i32 { a + b }", "markdown");
    ...
    info!("  - Review Prompt: {} ({})", review_prompt.name(), review_prompt.description().unwrap_or("No description"));
    info!("  - Docs Prompt: {} ({})", docs_prompt.name(), docs_prompt.description().unwrap_or("No description"));
    info!("  - Error Prompt: {} ({})", error_prompt.name(), error_prompt.description().unwrap_or("No description"));
    info!("  - Plan Prompt: {} ({})", plan_prompt.name(), plan_prompt.description().unwrap_or("No description"));
    
    // Test prompt argument schemas
    info!("📝 Testing Prompt Arguments:");
    info!("  - Review prompt args: {:?}", review_prompt.arguments().map(|args| args.len()));
    info!("  - Docs prompt args: {:?}", docs_prompt.arguments().map(|args| args.len()));
    info!("  - Error prompt args: {:?}", error_prompt.arguments().map(|args| args.len()));
    info!("  - Plan prompt args: {:?}", plan_prompt.arguments().map(|args| args.len()));
    ...
    info!("✅ MultiContentPrompt: {} messages with all ContentBlock types", multi_messages.len());
    ...
    for (i, message) in multi_messages.iter().enumerate() {
        ...
        info!("   Message {}: {} content - Role: {:?}", i + 1, content_type, message.role);
    }
    ...
    info!("✅ TemplateVarPrompt: {} messages with variable substitution", template_messages.len());
    ...
    info!("✅ TemplateVarPrompt with args override: {} messages", override_messages.len());
    ...
    if multi_messages.len() > 1 {
        if let turul_mcp_protocol::prompts::ContentBlock::ResourceLink { resource } = &multi_messages[1].content {
    ...
    if multi_messages.len() > 2 {
        if let turul_mcp_protocol::prompts::ContentBlock::Image { data, mime_type } = &multi_messages[2].content {
    ...
    if let Some(resource_msg) = multi_messages.iter().find(|msg| matches!(msg.content, turul_mcp_protocol::prompts::ContentBlock::Resource { .. })) {
        if let turul_mcp_protocol::prompts::ContentBlock::Resource { resource, annotations, meta } = &resource_msg.content {
