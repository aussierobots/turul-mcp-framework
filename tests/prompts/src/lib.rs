...
pub struct MultiContentPrompt {
    #[argument(name = "analysis_type", description = "Type of analysis to perform", required = true)]
    pub analysis_type: String,
    
    #[argument(name = "include_chart", description = "Whether to include chart visualization")]
...
impl MultiContentPrompt {
    ...
    pub async fn generate_multi_content_messages(&self) -> McpResult<Vec<PromptMessage>> {
        use turul_mcp_protocol::prompts::{ContentBlock, ResourceReference, ResourceContents};
        use serde_json::json;
...
pub struct TemplateVarPrompt {
    ...
    #[argument(name = "task_type", description = "Type of task to perform", required = true)]
