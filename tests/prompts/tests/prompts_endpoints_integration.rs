...
use turul_mcp_server::handlers::{PromptsListHandler, PromptsGetHandler, McpHandler};
...
use turul_mcp_protocol::prompts::{
    ...
    HasPromptAnnotations, HasPromptMeta, PromptArgument
};
...
async fn test_prompts_list_endpoint_integration() {
    ...
    assert!(first_prompt.description.as_ref().unwrap().contains("integration testing"));
}
...
async fn test_prompts_list_pagination_integration() {
    ...
    for page1_name in &page1_names {
        assert!(!page2_names.contains(page1_name), "Pages should not overlap");
    }
}
...
async fn test_prompts_get_endpoint_integration() {
    ...
    assert_eq!(get_result.description.as_ref().unwrap(), "A prompt for testing the get endpoint");
...
async fn test_prompts_get_with_meta_propagation() {
    ...
    let prompt = SimpleTestPrompt::new(
        "meta_test_prompt",
        "A prompt for testing _meta propagation"
    );
    handler = handler.add_prompt_arc(std::sync::Arc::new(prompt));
    ...
    assert_eq!(response_meta.get("request_id").unwrap(), &json!("req_12345"));
}
...
async fn test_prompts_get_missing_required_arguments() {
    ...
    let prompt = SimpleTestPrompt::new(
        "required_args_prompt",
        "A prompt that requires arguments"
    ).with_required_arg("topic").with_required_arg("style");
    ...
    assert!(result.is_err(), "Should return error for missing required arguments");
...
async fn test_prompts_get_nonexistent_prompt() {
    ...
    let result = handler.handle(Some(get_params)).await;
    assert!(result.is_err(), "Should return error for nonexistent prompt");
