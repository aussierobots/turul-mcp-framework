...
use turul_mcp_server::handlers::{PromptsGetHandler, McpHandler};
...
async fn test_partial_required_arguments() {
    ...
    match error {
        ...
        _ => panic!("Expected InvalidParameters error for missing required_arg2, got: {:?}", error)
    }
}
...
async fn test_all_required_arguments_provided() {
    ...
    let result = handler.handle(Some(get_params)).await;
    assert!(result.is_ok(), "Should succeed when all required arguments are provided");
...
async fn test_optional_arguments_handling() {
    ...
    let prompt = ValidatedTestPrompt::new(
        "optional_args_prompt",
        "A prompt with optional arguments"
    )
    ...
    let result = handler.handle(Some(get_params)).await;
    assert!(result.is_ok(), "Should succeed when only required arguments are provided");
    ...
    assert!(result.is_ok(), "Should succeed with optional arguments included");
}
...
async fn test_prompt_without_arguments_schema() {
    ...
    let result = handler.handle(Some(get_params)).await;
    assert!(result.is_ok(), "Prompt without argument schema should work with no arguments");
    ...
    assert!(result.is_ok(), "Prompt without argument schema should accept any arguments");
}
...
async fn test_empty_string_arguments_handling() {
    ...
    let result = handler.handle(Some(get_params)).await;
    assert!(result.is_ok(), "Empty string should satisfy required argument check");
}
...
async fn test_null_arguments_handling() {
    ...
    let result = handler.handle(Some(get_params)).await;
    assert!(result.is_err(), "Null arguments should trigger missing required argument error");
}