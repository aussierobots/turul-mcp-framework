...
use mcp_prompts_tests::{
    AnalyzeErrorPrompt, GenerateDocsPrompt, PlanProjectPrompt, ReviewCodePrompt,
};
use turul_mcp_protocol::prompts::{ContentBlock, Prompt, PromptMessage, Role};
...
async fn test_prompt_message_mcp_role_compliance() {
    ...
    let has_user_message = messages.iter().any(|msg| matches!(msg.role, Role::User));
    assert!(
        has_user_message,
        "Prompt should have at least one User message"
    );
}
...
async fn test_prompt_template_variable_substitution_mcp_compliance() {
    let review_prompt =
        ReviewCodePrompt::new("python", "def hello(): pass").with_focus("performance");
...
async fn test_prompt_business_logic_methods_coverage() {
    ...
    let review_prompt =
        ReviewCodePrompt::new("typescript", "const x = 1;").with_target_level("senior");
...
async fn test_prompt_polymorphism_mcp_compliance() {
    ...
    for prompt in &prompts {
        ...
        assert_eq!(
            prompt_struct.description,
            prompt.description().map(String::from)
        );
...
async fn test_prompt_mcp_error_handling_compliance() {
    ...
    let json_rpc_error = prompt_error.to_error_object();
