...
async fn test_high_concurrency_client_creation() {
    ...
    info!("Starting high-concurrency test with {} clients", client_count);
    ...
    for result in results {
        ...
        assert!(session_set.insert(session_id.clone()), 
               "Session ID {} for client {} should be unique", session_id, client_id);
        
        init_times.push(init_duration);
        debug!("Client {} completed with session {} in {:?}", client_id, session_id, init_duration);
    }
    ...
    info!("  Min/Max init time: {:?} / {:?}", min_init_time, max_init_time);
    info!("  All session IDs unique: {}", session_set.len() == client_count);
    
    // Performance assertions
    assert!(avg_init_time < Duration::from_secs(5), "Average init time should be reasonable");
    assert!(session_set.len() == client_count, "All sessions should be unique");
}
...
async fn test_resource_contention_isolation() {
    ...
    assert_eq!(unique_sessions.len(), client_count, "All sessions should be unique");
            ...
            for resource_uri in resources {
                for attempt in 0..3 {
                    ...
                    match result {
                        ...
                        Err(e) => {
                            client_results.push((resource_uri.to_string(), attempt, false, Some(e.to_string())));
                        }
                    }
        ...
        for (resource, _attempt, success, error) in operations {
            ...
            if success {
                ...
                if err.to_lowercase().contains("session") {
                    session_errors += 1;
                    warn!("Session error for client {} on {}: {}", client_id, resource, err);
                }
            }
        }
        
        debug!("Client {} ({}) completed all resource access operations", client_id, session_id);
    }
    ...
    assert!(success_rate > 70.0, "Should have reasonable success rate under contention");
...
async fn test_long_running_session_persistence() {
    ...
    info!("Testing long-running operations with {} clients for {:?}", client_count, operation_duration);
    ...
    for client_id in 0..client_count {
        ...
        let handle = tokio::spawn(async move {
            ...
            while start_time.elapsed() < duration {
                ...
                match operation_result {
                    Ok(_) => {
                        ...
                        assert_eq!(client.session_id().unwrap(), &initial_session, 
                                 "Session should remain consistent during long operations");
                        operation_count += 1;
                    }
                    Err(e) => {
                        if e.to_string().to_lowercase().contains("session") {
                            panic!("Session error during long operation for client {}: {}", client_id, e);
                        }
            ...
            (client_id, initial_session, final_session, operation_count, total_duration)
        });
...
async fn test_cross_protocol_session_management() {
    ...
    info!("Testing cross-protocol session management with {} client pairs", client_pairs);
    ...
    for pair_id in 0..client_pairs {
        ...
        let handle = tokio::spawn(async move {
            ...
            assert_ne!(resource_session, prompts_session, 
                      "Different protocol sessions should be unique for pair {}", pair_id);
            ...
            for _op in 0..10 {
                ...
                if resource_result.is_ok() {
                    ...
                    assert_eq!(resource_client.session_id().unwrap(), &resource_session,
                ...
                if prompts_result.is_ok() {
                    ...
                    // Verify session consistency
                    assert_eq!(prompts_client.session_id().unwrap(), &prompts_session,
            ...
            (pair_id, resource_session, prompts_session, operations_completed)
        });
    ...
    for result in results {
        ...
        assert!(all_resource_sessions.insert(resource_session.clone()),
               "Resource session {} for pair {} should be unique", resource_session, pair_id);
        assert!(all_prompts_sessions.insert(prompts_session.clone()),
               "Prompts session {} for pair {} should be unique", prompts_session, pair_id);
        
        total_operations += operations;
        
        debug!("Pair {} completed {} operations (Resource: {}, Prompts: {})", 
    ...
    info!("  Unique resource sessions: {}", all_resource_sessions.len());
    info!("  Unique prompts sessions: {}", all_prompts_sessions.len());
    info!("  Total operations: {}", total_operations);
    
    // All sessions should be unique within each protocol
    assert_eq!(all_resource_sessions.len(), client_pairs, "All resource sessions should be unique");
    assert_eq!(all_prompts_sessions.len(), client_pairs, "All prompts sessions should be unique");
    assert!(total_operations > 0, "Should have completed some operations");
}
...
async fn test_session_cleanup_under_load() {
    ...
    info!("Testing session cleanup with {} waves of {} clients", waves, clients_per_wave);
    
    let all_session_ids = Arc::new(Mutex::new(Vec::new()));
    
    for wave in 0..waves {
        info!("Starting wave {} with {} clients", wave + 1, clients_per_wave);
        ...
        for result in wave_results {
            ...
            debug!("Wave {} client {} completed with session {}", wave_num, client_id, session_id);
        }
    ...
    assert_eq!(session_ids.len(), total_expected, "Should have created expected number of sessions");
    assert_eq!(unique_sessions.len(), total_expected, "All sessions should be unique");
