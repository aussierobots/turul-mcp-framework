...
async fn test_session_id_generation_and_persistence() {
    ...
    let prompts_session = prompts_client
        .session_id()
        .expect("Prompts client should have session ID");
    ...
    assert_ne!(
        ...
        "Different servers should generate different session IDs"
    );
}
...
async fn test_cross_request_session_consistency() {
    ...
    for (method, params) in operations {
        ...
        assert_eq!(
            ...
            &initial_session,
            "Session ID should not change during requests"
        );
    }
...
async fn test_session_isolation_between_clients() {
    ...
    assert_ne!(
        session1, session2,
        "Client1 and Client2 should have different sessions"
    );
    assert_ne!(
        session1, session3,
        "Client1 and Client3 should have different sessions"
    );
    assert_ne!(
        session2, session3,
        "Client2 and Client3 should have different sessions"
    );
        ...
        if result.contains_key("result") {
            ...
            if !contents.is_empty() {
                ...
                assert!(
                    ...
                    "Session resource should reference session context"
                );
                info!(
                    "Session-aware resource call {}: {}",
                    i,
                ...
                if let Some(_session_id) = client.session_id() {
                    ...
                    assert!(
                        text.contains("session") || text.len() > 10,
                        "Should have meaningful session content"
                    );
                }
            }
        }
...
async fn test_session_aware_prompt_context() {
    ...
    for i in 1..=3 {
        let result = client
            ...
            .await
            .expect("Failed to get session prompt");
        ...
        if result.contains_key("result") {
            ...
            if !messages.is_empty() {
                ...
                assert!(
                    ...
                    "Session-aware prompt should reference session context"
                );
                info!(
                    "Session-aware prompt call {}: {}",
                    i,
                    message_content.lines().next().unwrap_or("")
                );

                // Should be consistent but potentially different each call
                assert!(
                    message_content.len() > 10,
                    "Should have meaningful session content"
                );
            }
        }
    ...
    for i in 0..clients.len() {
        for j in i + 1..clients.len() {
            assert_ne!(
                ...
                "Clients {} and {} should have different sessions",
                i,
                j
            );
        }
    ...
    for (i, client) in clients.into_iter().enumerate() {
        let handle = tokio::spawn(async move {
            ...
            assert_eq!(
                client.session_id().unwrap(),
                &session_id,
                "Session should not change during operations"
            );
...
async fn test_session_header_propagation() {
    ...
    for (method, params) in requests {
        ...
        match result {
            Ok(response) => {
                info!(
                    "✅ Request {} succeeded with session {}",
                    method, session_id
                );
                ...
                if let Some(error) = response.get("error") {
                    ...
                    assert!(
                        !error_msg.to_lowercase().contains("session"),
                        "Should not have session errors: {}",
                        error_msg
                    );
                }
            }
...
async fn test_session_management_comprehensive() {
    ...
    SessionTestUtils::verify_session_consistency(&resource_client)
        .await
        .expect("Resource session consistency failed");
    SessionTestUtils::verify_session_consistency(&prompts_client)
        .await
        .expect("Prompts session consistency failed");

    SessionTestUtils::test_session_aware_resource(&resource_client)
        .await
        .expect("Session-aware resource test failed");
    SessionTestUtils::test_session_aware_prompt(&prompts_client)
        .await
