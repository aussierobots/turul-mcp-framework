...
fn main() {
    ...
    let calculator = Tool::new(
        ...
        ToolSchema::object()
            .with_properties(HashMap::from([
                ("expression".to_string(), JsonSchema::string()),
            ]))
    ...
    let full_request = CreateMessageRequest::new(
        vec![
            SamplingMessage::user_text("Read the config file and summarize its contents."),
        ],
        4096,
    )
    .with_tools(vec![file_read, web_search, calculator])
    .with_system_prompt("You are a code analysis assistant with access to file reading and search tools.")
    ...
    let no_tools_request = CreateMessageRequest::new(
    ...
    println!("4. Request without tools (tools field omitted: {}):", !has_tools_field);
    println!("{}\n", json);
