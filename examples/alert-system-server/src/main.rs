    ...
    fn new() -> Self {
        let input_schema = ToolSchema::object()
            .with_properties(HashMap::from([
                (
                    "name".to_string(),
                    JsonSchema::string().with_description("Alert rule name"),
                ),
                (
                    "description".to_string(),
                    JsonSchema::string().with_description("Alert rule description"),
                ),
                (
                ...
                (
                    "condition_config".to_string(),
                    JsonSchema::object().with_description("Condition-specific configuration"),
                ),
                (
                    "severity".to_string(),
                    JsonSchema::string_enum(vec![
                ...
                (
                    "channels".to_string(),
                    JsonSchema::array(JsonSchema::string_enum(vec![
                        "email".to_string(),
                        "slack".to_string(),
                        "webhook".to_string(),
                    ]))
...
impl McpToolTrait for ConfigureAlertRuleTool {
    async fn call(
        ...
        session
            .notify_progress(format!("alert_rule_created_{}", severity.to_lowercase()), 1)
            .await;
    ...
    fn new() -> Self {
        let input_schema = ToolSchema::object()
            .with_properties(HashMap::from([
                (
                    "data".to_string(),
                    JsonSchema::object().with_description("Data to check against alert conditions"),
                ),
                (
                    "data_type".to_string(),
                    JsonSchema::string_enum(vec![
...
impl McpToolTrait for CheckAlertConditionsTool {
    async fn call(
        ...
        let auto_acknowledge = args
            .get("auto_acknowledge")
            .and_then(|v| v.as_bool())
            .unwrap_or(false);
...
impl GetAlertHistoryTool {
    fn new() -> Self {
        let input_schema = ToolSchema::object().with_properties(HashMap::from([
            ...
            (
                "acknowledge_alert_id".to_string(),
                JsonSchema::string().with_description("Alert ID to acknowledge"),
            ),
            (
                "acknowledged_by".to_string(),
                JsonSchema::string().with_description("Person acknowledging the alert"),
            ),
        ]));
        Self { input_schema }
    }
...
impl McpToolTrait for GetAlertHistoryTool {
    async fn call(
        ...
        let acknowledged_filter = args
            .get("acknowledged_filter")
            .and_then(|v| v.as_str())
            .unwrap_or("all");
