...
use mcp_server::{McpServer, McpResult, McpHandler};
use mcp_protocol::{completion::{CompletionRequest, CompletionResponse, CompletionSuggestion}, McpError};
...
impl IdeCompletionHandler {
    ...
    pub fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let file_extensions = vec![
            ...
            ".ts".to_string(), ".java".to_string(), ".go".to_string(),
            ".cpp".to_string(), ".c".to_string(), ".kt".to_string(),
            ".swift".to_string(), ".php".to_string(), ".rb".to_string(),
            ".json".to_string(), ".yaml".to_string(), ".toml".to_string(),
            ".md".to_string(), ".txt".to_string(), ".html".to_string(),
            ".css".to_string(), ".scss".to_string(), ".sql".to_string(),
        ];
        
        info!("Loaded {} languages, {} frameworks, {} commands", 
              languages.len(), frameworks.len(), commands.len());
    ...
    fn load_commands(data_dir: &Path) -> Result<Vec<DevelopmentCommand>, Box<dyn std::error::Error>> {
    ...
    fn get_extension_completions(&self, prefix: &str) -> Vec<CompletionSuggestion> {
        self.file_extensions
            ...
            .map(|ext| CompletionSuggestion {
                ...
                description: Some(match ext.as_str() {
                    ".rs" => "Rust source file",
    ...
    fn get_command_completions(&self, prefix: &str) -> Vec<CompletionSuggestion> {
        self.commands
            ...
            .map(|cmd| CompletionSuggestion {
                ...
                description: Some(format!("{} - Tools: {}", cmd.description, cmd.common_tools.join(", "))),
            })
            .collect()
    }
    ...
    fn get_framework_completions(&self, prefix: &str) -> Vec<CompletionSuggestion> {
        self.frameworks
            ...
            .map(|fw| CompletionSuggestion {
                ...
                description: Some(format!("{} ({} - {})", fw.description, fw.language, fw.category)),
            })
            .collect()
    }
    ...
    fn get_smart_completions(&self, argument_name: &str, current_value: &str) -> Vec<CompletionSuggestion> {
        ...
        match argument_name.to_lowercase().as_str() {
            ...
            "extension" | "ext" | "file_extension" => {
                self.get_extension_completions(&prefix)
            },
            "command" | "cmd" | "action" => {
    ...
    async fn handle(&self, params: Option<Value>) -> McpResult<Value> {
        if let Some(params) = params {
            let request: CompletionRequest = serde_json::from_value(params)
                .map_err(|e| McpError::invalid_param_type("params", "CompletionRequest", &e.to_string()))?;
            
            let suggestions = self.get_smart_completions(
            ...
            serde_json::to_value(response).map_err(|e| McpError::tool_execution(&format!("Failed to serialize completion response: {}", e)))
        } else {
            ...
            let suggestions = vec![
                CompletionSuggestion {
                    ...
                    description: Some("Systems programming language focused on safety and performance".to_string()),
                },
                CompletionSuggestion {
                    value: "python".to_string(),
                    label: Some("Python programming language".to_string()),
                    description: Some("High-level programming language with elegant syntax".to_string()),
                },
                CompletionSuggestion {
                    value: "javascript".to_string(),
                    label: Some("JavaScript programming language".to_string()),
                    description: Some("Dynamic programming language for web development".to_string()),
                },
            ];
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("  - Programming language suggestions with categories (language, lang, programming_language)");
