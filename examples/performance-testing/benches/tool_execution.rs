...
#[tool(
    ...
    description = "Simple computation for benchmarking"
)]
...
#[tool(
    name = "benchmark_cpu",
    description = "CPU-intensive computation for benchmarking"
)]
...
#[tool(
    ...
    description = "Session-aware tool for benchmarking"
)]
...
impl BenchmarkSessionTool {
    async fn execute(&self, session: Option<SessionContext>) -> McpResult<String> {
        if let Some(session) = session {
            ...
            let current: i64 = current_value.and_then(|v| v.as_i64()).unwrap_or(0);
...
fn tool_execution_benchmarks(c: &mut Criterion) {
    ...
    for size in [10, 50, 100, 500].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("cpu_intensive", size), size, |b, &size| {
            b.to_async(&rt).iter(|| async {
