...
impl MemorySnapshot {
    ...
    fn print(&self, label: &str) {
        ...
        info!(
            "Total allocated: {}",
            Self::format_bytes(self.total_allocated_bytes)
        );
...
async fn send_memory_request(
    ...
    let results = client.call_tool(tool_name, params).await?;

    // Convert tool results to simple value for compatibility
    Ok(serde_json::to_value(results)?)
}

async fn baseline_memory_test(client: &McpClient, request_count: usize) -> anyhow::Result<()> {
    info!(
        "Starting baseline memory measurement with {} requests",
        request_count
    );
    ...
    // Analysis
    let memory_per_request = (final_snapshot.current_usage_bytes as i64
        - warmup.current_usage_bytes as i64)
        / request_count as i64;
    info!("=== Baseline Analysis ===");
    info!("Memory per request: {} bytes", memory_per_request);
    info!(
        "Memory efficiency: {:.2} requests per MB",
...
async fn leak_detection_test(client: &McpClient, iterations: usize) -> anyhow::Result<()> {
    info!(
        "Starting memory leak detection with {} iterations",
        iterations
    );
    ...
    if snapshots.len() >= 3 {
        ...
        let sum_xy: f64 = snapshots
            .iter()
            .enumerate()
            .map(|(i, s)| i as f64 * s.current_usage_bytes as f64)
            .sum();
...
async fn growth_analysis_test(
    ...
    info!(
        "Starting memory growth analysis over {} seconds",
    ...
    if snapshots.len() >= 2 {
        ...
        info!(
            "Total memory growth: {}",
            MemorySnapshot::format_bytes(total_growth.unsigned_abs() as usize)
        );
        info!(
            "Growth rate: {:.2} bytes per minute",
            growth_rate_per_minute
        );

        // Find peak usage
        let peak_snapshot = snapshots
            .iter()
            .max_by_key(|s| s.current_usage_bytes)
            .unwrap();
        info!(
            "Peak usage: {} at {:?}",
