...
| Origin absent → allowed; loopback → allowed; **a matching `Host` header does NOT admit a foreign origin** | MUST | Implemented | `c/turul-http-mcp-server/src/origin.rs` | `origin_validation_2026.rs::origin_absent_is_allowed`, `::loopback_origin_is_allowed_by_default`, `::servers_own_loopback_origin_is_allowed_by_default`, `origin.rs::matching_host_header_does_not_admit_a_foreign_origin` | pass | — | — | — |
