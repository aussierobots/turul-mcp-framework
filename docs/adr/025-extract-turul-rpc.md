...
- **2026-06-10 (framework shim cut LANDED)** — The four non-frozen framework crates (`turul-mcp-server`, `turul-http-mcp-server`, `turul-mcp-builders`, `turul-mcp-aws-lambda`) now depend on `turul-rpc` directly (146 path swaps; the shim's path layout mirrors `turul-rpc` 1:1, and the re-exported types are nominally identical, so no public API changed). The 2026-default `middleware-auth-lambda` example dropped its unused shim dep. The shim crate remains in the workspace solely for the FROZEN `turul-mcp-protocol-2025-06-18`/`-2025-11-25` snapshots and the 2025-pinned test/example crates, and its manifest version is restored to the terminal `0.3.47` line (the mechanical 0.4.0 sweep value was never publishable; the workspace pin now says `0.3.47`). Outcome matches this ADR's decision: framework 0.4.0 ships without the shim; no 0.4 of the shim exists or will be published.
