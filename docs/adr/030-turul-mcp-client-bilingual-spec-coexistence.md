...
- **2026-06-10 (alias dependency removed; SEP-2243 headers; -32004 negotiation signal)** — Three contract changes landed in the header-enforcement slice:
  1. **The `turul-mcp-protocol` alias dependency is gone from the client manifest** — the as-built crate had retained it (pinned to `protocol-2025-11-25`) as the lingua-franca type surface, contradicting this ADR's "the client does NOT route through the alias" design and (the practical trigger) making it impossible to link the bilingual client and a 2026-default server into one binary: dependency unification enabled both alias protocol features and tripped the ADR-029 spec mutex. The frozen `turul-mcp-protocol-2025-11-25` crate is now an **unconditional** dependency serving as the public type vocabulary (`Tool`, `CallToolResult`, …) for every API regardless of negotiated spec; `turul-mcp-protocol-2026-07-28` remains feature-gated (`client-bilingual` / `client-2026-07-28-only`). Consequence for the narrowing features: they now control which WIRE paths compile, not which type crates link (`client-2025-11-25-only = []`).
  2. **SEP-2243 request-metadata headers are emitted on 2026 connections**: `Mcp-Method` on every request/notification, `Mcp-Name` on `tools/call`/`prompts/get` (`params.name`) and `resources/read` (`params.uri`). The `server/discover` probe now advertises `MCP-Protocol-Version: 2026-07-28` (its `_meta` already said so; the header MUST match the body — probing with the legacy header is rejected 400 by a validating 2026 server). The `FallbackTo2025` arm restores the legacy header before `initialize`.
  3. **`-32004 UnsupportedProtocolVersionError` is now a fallback trigger**: a 400 whose body parses as a JSON-RPC error surfaces its code to `classify_probe`, and `-32004` → fall back to 2025 (the server's structured "I don't speak the requested 2026"). This narrowly extends the 2026-05-31 codex P1-5 rule — the trusted signals are now `-32601` (no such method → pre-2026 server) and `-32004` (validated and declined → version negotiation), both structured JSON-RPC errors; bare 4xx still aborts without downgrade, and `allow_legacy_gateway_fallback` is unchanged. This matches the live draft's Backward Compatibility guidance ("on 400, inspect the body … a recognized modern JSON-RPC error means the server speaks a modern version") while remaining stricter than its "fall back on any unrecognized 400 body" clause.
  - Acceptance: `tests/e2e_2026_real_server.rs` (bilingual client ↔ REAL in-process 2026 `turul-mcp-server`: negotiation + `tools/list` + `tools/call`, failing before the probe-header fix), strengthened wiremock matchers in `tests/bilingual_2026_operations.rs` (stubs require `Mcp-Method`/`Mcp-Name`/`MCP-Protocol-Version`), `-32004` classifier unit test. The dev-dependency set gained `turul-mcp-server`/`turul-mcp-derive`/`turul-mcp-protocol`/`turul-mcp-builders` (test targets only; the published library links none of them).

- **2026-06-10 (MRTR client arm landed)** — closes the "Still pending: MRTR `InputRequiredResult`" item from the 2026-06-07 entry. `call_tool` on a 2026 connection surfaces `resultType: "input_required"` as `McpClientError::InputRequired { input_requests, request_state }`; the application gathers inputs and retries with `McpClient::call_tool_with_input_responses` (original name/arguments + `inputResponses` + verbatim `requestState`, fresh JSON-RPC id). Capability truthfulness: the new `ClientConfig.declared_capabilities` (elicitation/sampling/roots, default off) feeds the per-request 2026 `_meta` `clientCapabilities` AND the 2025 `initialize` capabilities — both were previously hardcoded empty, which would have made any spec-honest MRTR server reject elicit requests with `-32003`. Acceptance: client-driven two-leg MRTR e2e against a real 2026 server in `tests/e2e_2026_real_server.rs`. Still pending from the 2026-06-07 list: `completion/complete` client op; client-side `subscriptions/listen` (closed by the next entry).

- **2026-06-11 (MRTR client arm completed for resources/read + prompts/get; resultType discipline)** — Closes spec-compliance driver gaps CF/GAP-CF-3, PRM/PR-2026-02, RES-G1, PAT/G3, BP-1. `parse_read_resource` and `parse_get_prompt` now surface `resultType: "input_required"` as `McpClientError::InputRequired` (previously a serde "missing field" error that discarded `inputRequests`/`requestState`), with retry APIs `read_resource_with_input_responses` / `get_prompt_with_input_responses` mirroring the tools/call arm. The former `input_required_outcome` helper became `check_result_type`, applied across ALL 2026 result parsers, and enforces basic §Responses: "A resultType of any value unrecognized by the client MUST be considered invalid" (unknown discriminator → `ProtocolError::InvalidResponse`, no longer silently treated as complete). Acceptance: real-server e2e `mrtr_round_trip_on_resources_read_through_the_client` / `mrtr_round_trip_on_prompts_get_through_the_client` (revert-and-fail recorded: stripping the parser checks reproduces the serde-error mis-parse) + module unit tests for the discriminator contract.

- **2026-06-10 (client-side `subscriptions/listen` landed)** — closes the last functional item from the 2026-06-07 list except `completion/complete` (a dedicated client op remains unbuilt; the server-side `completion/complete` surface is e2e-covered). `McpClient::subscriptions_listen(filter)` opens the long-lived POST SSE stream on a 2026 connection via the new `Transport::send_request_streaming` (default impl rejects on transports without streaming support; `HttpTransport` parses SSE frames into a channel-backed `SubscriptionStream`). The stream surfaces the ack-first `notifications/subscriptions/acknowledged` (honored filter) and stamps every delivered notification's `io.modelcontextprotocol/subscriptionId`. Acceptance: real-server listen e2e in `tests/e2e_2026_real_server.rs`.

- **2026-07-13 (recognized-modern-error set corrected for the post-renumbering wire; two stale-code regressions fixed)** — The recognized-modern-error set for probe classification (`classify_probe`) is now explicit: `{-32022 UnsupportedProtocolVersionError, -32021 MissingRequiredClientCapabilityError, -32020 HeaderMismatch}`, matching the live draft Streamable HTTP §Backward Compatibility guidance. Pre-renumbering draft codes (`-32004`, `-32001`) are deliberately **unrecognized — no legacy aliases**: `-32001`/`-32004` fall in the post-renumbering implementation-defined range, and `-32001` is this framework's own middleware code for `Unauthenticated`, so aliasing it to a version signal would misclassify a real auth failure as "server speaks a different spec." The abort-on-unrecognized-400 downgrade-resistance posture from the 2026-06-10 entry is unchanged — an unrecognized code still aborts rather than falling back. Two regressions found and fixed in the same slice: (1) `call_tool`'s 2026-lane Mcp-Param mismatch auto-retry was still keyed on `-32001` instead of `ERROR_CODE_HEADER_MISMATCH` (`-32020`); (2) the classifier's own `UnsupportedProtocolVersionError` constant was still `-32004` instead of `-32022`. Root cause: `bilingual_negotiation.rs` mocked the pre-renumbering codes (faithful-mock rot after the 2026-07-02 schema renumbering — see CHANGELOG/schema README), and the real-server e2e suite never exercised a version-mismatch path. Tests: `call_tool_recovers_from_header_mismatch_with_one_refresh_and_retry` (wire, one refresh + one retry), `pre_renumbering_32004_is_unrecognized_and_aborts` (unit), `bilingual_client_treats_pre_renumbering_32004_as_unrecognized_and_aborts` (wire); `bilingual_client_falls_back_on_400_with_32004_body` renamed to `bilingual_client_falls_back_on_400_with_32022_body`. Revert-and-fail recorded for both fixes.

- **2026-07-29 (SEP-2106 schema downgrade instead of listing failure; `completion/complete` + `notifications/cancelled`; GET SSE deferred and skipped on 2026; phantom transport features deleted)** — Four changes, all in `turul-mcp-client`:
  1. **`list_tools()` no longer hard-fails on a JSON Schema 2020-12 composition.** The public client vocabulary is the frozen `turul-mcp-protocol-2025-11-25` types (2026-06-10 entry item 1), whose `ToolSchema.properties` is `HashMap<String, JsonSchema>` and whose `JsonSchema` is an internally-tagged enum on `"type"` with no fallback arm. A property written as `{"oneOf": […]}` — legal on 2026 `inputSchema` per SEP-2106, and what this framework's own server emits for a data-bearing tagged union — therefore had no representation, and because `parse_list_tools` collected into a `Result`, ONE such tool errored the ENTIRE listing. Turul-client could not list tools from turul-server on the revision's headline schema change. The frozen crate cannot be widened, so the remap is now **infallible by construction**: a direct remap is tried first, and on failure the tool is rebuilt field by field — unrepresentable `properties` entries are dropped individually, an `outputSchema` with a non-object root is dropped whole, and each dropped path is named in a `tracing::warn!`. **Cost, stated plainly:** a caller reading `Tool.input_schema` on a 2026 connection may see a schema that is missing properties the server advertised, and `required` still names them. That is deliberate — silently dropping a valid, callable tool is a worse failure than losing schema detail, because the caller cannot tell it happened. The loss is recoverable: the untruncated advertised schema is retained per tool and exposed as `McpClient::tool_input_schema(name)`, and the caveat is documented on `list_tools`/`refresh_tools`/`list_tools_paginated`. The alternative considered and rejected was excluding the tool from the listing (the existing treatment for a SEP-2243 x-mcp-header violation or a dialect-invalid schema) — those are definitions a client MUST NOT act on, whereas a 2020-12 composition is *valid* and merely inexpressible in our public type.
  2. **`completion/complete` and `notifications/cancelled` are reachable from `McpClient`.** `complete(reference, argument, context)` routes bilingually (2026 lane via the per-request `_meta` path); the result is built field by field because `CompletionResult.total` is `f64` on the 2026 wire and `u32` in the public vocabulary, so an integral `100.0` would not survive a serde round trip. `cancel_request(request_id, reason)` sends the spec-neutral `notifications/cancelled` (dropping a request already closes its stream and the server treats that as cancellation; this is the explicit form, and the only one that can carry a reason). `SubscriptionStream::request_id()` was added so a caller has at least one request id it can legitimately name.
  3. **The GET SSE listener is deferred until after negotiation, and skipped entirely on 2026-07-28.** It previously spawned inside `connect()` *before* negotiation resolved, so every 2026 connection issued a GET that the revision deleted, took an HTTP 405, and logged warnings naming `initialize` and session ids — both dead concepts on that lane. The listener now starts after `negotiate_protocol()`; `HttpTransport::start_event_listener` also refuses to open the stream on a 2026 connection. Deferring additionally removes the pre-existing race the old code compensated for with a compare-and-swap: the 2025-11-25 GET now carries the session id the handshake produced (the revert-and-fail run showed the old ordering failing that assertion too). The three "…without session ID" warnings on the POST request, POST notification, and GET paths are version-gated so they fire only where the header actually exists.
  4. **Phantom `stdio` / `all-transports` Cargo features deleted.** Both were declared in the manifest with no stdio module behind them (the module has always been commented out); `detect_transport_type` already returns `TransportError::Unsupported` for a `stdio://` URL, so nothing regresses. `tokio-util` stays — it is still pulled by `sse` and used by the HTTP transport.
  - Acceptance: `tests/schema_2020_12_tools_list.rs` (real in-process 2026 server advertising a `oneOf` property → listing succeeds, tool present, tool callable, raw schema recoverable; paginated variant too), `tests/completion_and_cancellation_2026.rs` (real server, provider-driven completion values, context pass-through, cancellation accepted), `tests/get_sse_listener_lifecycle.rs` (0 GETs on a 2026 connection; the 2025 GET still opens carrying its session id), plus module unit tests for the typeless-`outputSchema` and sibling-property cases. Revert-and-fail recorded for (1) and (3): reverting the parser reproduces `Json(Error("missing field \`type\`"))` on both `list_tools` and `list_tools_paginated`, and reverting the connect ordering reproduces both the stray 2026 GET and the sessionless 2025 GET.
