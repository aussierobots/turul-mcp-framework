...
### Wire-string target: `"DRAFT-2026-v1"`

The crate currently advertises and accepts the wire protocol version string exactly as the vendored draft schema declares it. This is reflected in:

- `crates/turul-mcp-protocol-2026-07-28/src/version.rs` — `McpVersion::V2026_07_28` serde-renames to `"DRAFT-2026-v1"`; `FromStr` parses `"DRAFT-2026-v1"`.
- `crates/turul-mcp-protocol-2026-07-28/src/lib.rs::MCP_VERSION` — `"DRAFT-2026-v1"`.
- `crates/turul-mcp-protocol-2026-07-28/schema/draft-schema.ts` — source-of-truth, vendored with ETag for provenance.

The Rust identifier `McpVersion::V2026_07_28` stays put. Only the serde rename and `MCP_VERSION` constant change when the wire string flips. The crate name itself (`turul-mcp-protocol-2026-07-28`) was chosen for the projected publication date and does not need to match the wire string.
