...
- **`protocol-2025-11-25` feature flag end-to-end coverage.** A small 2025-11-25 regression suite (tasks-e2e pair plus pinned logging/sampling/elicitation/client/lambda examples) is held at the 2025 opt-in, and the integration-test crates are pinned to it, exercising the fallback path. A full CI matrix that builds + tests the whole workspace with `--no-default-features --features protocol-2025-11-25` is the remaining proof-of-coverage item.
