...
- **CLAUDE.md "Test Coverage Discipline" rule 3** (wire-layer coverage requirement) is a permanent gate improvement, retained on `main` regardless of how this specific bug resolved. It exists because v0.3.42's failure mode was tests calibrated to the wrong layer; the rule prevents that recurring.
- **DRAFT-2026-v1 mode (default in 0.4.0)**: this ADR's resolution stands unchanged. The empty-body envelope failure is an AWS Lambda Runtime API + streaming-response framing concern, independent of MCP spec version. However, in DRAFT-2026-v1 mode the framework does not serve GET SSE (per ADR-006 amendment), so one of the contexts where empty-body responses get constructed (server-initiated notification keep-alives, ping responses, idle-stream heartbeats) does not arise. The remaining trigger — custom-route OPTIONS short-circuits with `Empty::new()` body — still applies if the consumer constructs such a response in a `run_streaming_with` dispatch closure, and the recommended workaround (APIGW MOCK on OPTIONS) is still the answer. No reopening of this ADR. No runtime change.
