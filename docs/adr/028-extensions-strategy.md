...
- **2026-07-29 (Apps spec re-vendored from the released dated path; pin gate extended to `ext-*` crates)** — The 2026-06-12 scaffold entry above records the Apps spec as "pinned at `modelcontextprotocol/ext-apps@ca1d2989`". That pin resolved `specification/draft/apps.mdx`, not the released `specification/2026-01-26/apps.mdx` the crate claimed to implement — the same floating-subpath hazard ADR-027's 2026-07-29 entry records for the core schema, reached independently in a second repository. **Misvendoring proven, not inferred**: re-fetching `specification/draft/apps.mdx` at `ca1d2989` reproduces the shipped bytes exactly.

  **Decision: `apps-2026-01-26.mdx` is normative, sourced from `specification/2026-01-26/apps.mdx` at `298e884ec3f02daba085acdb02042d73bd00b355` (tag `v1.0.0`)** — the commit that *created* that file, which upstream has never modified since, so the pin is immutable by construction rather than by SHA discipline alone.

  **Deviation from ADR-027's one-immutable-commit rule, taken deliberately.** `spec.types.ts` is pinned separately at `92f46a57` (tag `v1.7.5`). Upstream publishes no dated `specification/2026-01-26/spec.types.ts` — the SDK's own `src/spec.types.ts` is the only machine-readable form of these types that exists, and it keeps moving after the dated release ships. It is therefore recorded as a **convenience reference, not authority**: where it and the dated `.mdx` disagree, the `.mdx` wins. The split is safe because all six bound interfaces were diffed between the two commits — identical property sets and types, JSDoc prose only, plus post-release `never` guards the binding already documents. Reversible by fetching `src/spec.types.ts@298e884` and updating one table row.

  **The types were correct; a doc claim was not.** `UiResourceMeta` asserted "when present on both, the content-item value wins; hosts MUST check both locations." That MUST exists only in the draft's "Metadata Location" section, which the released 2026-01-26 document does not have. The comment was importing draft normativity into a released binding and is removed. Separately, the `EmptyObject` strictness this crate relies on comes from upstream `src/generated/schema.json` (`additionalProperties: false` per permission key), not from the `.mdx` or `.ts` — both spell it `camera?: {}`, which in TypeScript means "any non-nullish". That file is consulted, not vendored, and the constraint holds at both pinned commits; the provenance README records it so the claim is traceable.

  **Gate extended.** `scripts/check-schema-pin.sh` now parses the ext-apps provenance table and asserts three things offline: every listed file's sha256 matches disk, every `*.mdx` row's upstream source is a dated `specification/<YYYY-MM-DD>/` path, and every file in `schema/` appears in the table (so a stray copy cannot sit unchecked). The revert-and-fail run included a **well-formed** table honestly pointing at `specification/draft/` — the gate rejects it, which is the point: it catches the defect class, not the instance. `ext-tasks` gets the checksum arm only; upstream `modelcontextprotocol/ext-tasks` has no tags and only `schema/draft/`, so the dated-path rule genuinely cannot apply there yet.

  **Named tech debt.** The ext-tasks expected checksums live in the gate script rather than in `crates/turul-mcp-ext-tasks/schema/README.md`, which records none. Owner: `turul-mcp-ext-tasks`. Removal trigger: that README growing `Content sha256` cells, at which point the script reads from there and the in-script table is deleted.

  **Disclosure, not wiring.** `crates/turul-mcp-ext-apps/README.md` now states plainly that the crate is serde types and three helpers, is referenced by nothing in the framework, and does not enforce the Apps spec's `tools/list` visibility filtering or `tools/call` app-only rejection. Wiring it remains an open design decision, not a documentation gap.
