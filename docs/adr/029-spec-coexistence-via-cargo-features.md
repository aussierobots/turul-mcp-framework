...
3. Source updates in every consumer crate for the breaking surface deltas: `initialize` handshake → `server/discover`; `Mcp-Session-Id` header path → `_meta`-carried capability handshake; error code `-32002` → `-32602`; tasks calls routed to `turul-mcp-ext-tasks` (until then, tasks-using code is gated behind `protocol-2025-11-25`).
...
- **Aligns with ADR-028 extensions strategy.** Extensions (`turul-mcp-ext-tasks`, `turul-mcp-ext-apps`) are opt-in by cargo dependency. Spec-version selection is opt-in by cargo feature. Same idiom at both layers.
...
- **ADR-028 (Extensions strategy)** established that tasks live in `turul-mcp-ext-tasks` (not in the protocol crate). Under the default feature, framework code that used to call `tasks/list` etc. must move to the extension crate; under `protocol-2025-11-25`, tasks remain in core. The migration recipe in §"Migration from 2025-11-25 task users" of ADR-028 is the consumer guide.
...
- **`turul-mcp-ext-tasks` runtime wiring into `turul-mcp-server`.** Required for the default-feature build to compile any tasks-using consumer code via the extension crate. In the landed cutover, tasks are gated to the 2025-11-25 opt-in (the task runtime is `#[cfg(feature = "protocol-2025-11-25")]`); the extension crate is scaffolded (ADR-028, 2026-06-12) but not yet wired into `turul-mcp-server` dispatch. ADR-028 §"Implementation order" step 3.
...
- **2026-07-29 (2026-07-28 finalization)** — MCP 2026-07-28 has finalized and is now the released current specification. No change to this ADR's feature-gating mechanism; the `turul-mcp-ext-tasks` references above (Context item 4, §Open items, §Consequences) use the spec-neutral crate name adopted by ADR-028's 2026-06-07 amendment — see that entry for the rename from the earlier `-2026-07-28`-suffixed name.
...
- **2026-07-13 (dual-era MAY reviewed and declined; Lambda registration parity)** — Reviewed the live draft's newly formalized Backward-Compatibility section (Terminology: Modern/Legacy/Dual-era; compatibility matrix; "A dual-era server MAY serve both eras concurrently on the same endpoint or process"). Single-spec-per-build remains intentional: the MAY is declined; ecosystem coverage stays two build lanes (this ADR) plus the ADR-030 bilingual client. Upstream basis: commit `93671a3f2bac3bc11b0eb6327c2d029e272b2871`, schema sha256 `6e4cba2d17f7156877357762b6b4b63cd790d8973f61ec35ab73cd61ad67017d` — byte-identical to the vendored pin, verified 2026-07-13. Same slice: `turul-mcp-aws-lambda`'s dispatcher method-registration table synced to the non-Lambda authority per feature lane (see ADR-001 revision log, 2026-07-13) — a registration-set fix under this ADR's existing modern-only posture, not a change to it.
